...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="ML Pipeline Demo for TradingAgents-CN"
    )
    parser.add_argument(
        "--symbols",
        nargs="+",
...
def demo_model_training(symbols, model_type="xgboost"):
    ...
    try:
        ...
        print(
            f"📦 Creating dummy training data: {n_samples} samples, {n_features} features"
        )
        ...
        tasks = {
            ...
            "signal_classification": np.random.choice(
                ["BUY", "HOLD", "SELL"], n_samples
            ),
                ...
                if hasattr(model, "train"):
                    if task_name == "signal_classification":
                        ...
                        training_history = model.train(
                            pd.DataFrame(X), returns, volatility
                        )
                    else:
                        training_history = model.train(
                    ...
                    if (
                        isinstance(training_history, dict)
                        and "train_score" in training_history
                    ):
                        print(
                            f"     Train score: {training_history['train_score']:.4f}"
                        )

                else:
                    print(
                        f"⚠️  Model {model_type} does not support training for {task_name}"
                    )
        ...
        for task_name, model in trained_models.items():
            try:
                if hasattr(model, "predict"):
                    ...
                    print(
                        f"   {task_name}: {predictions[:3] if hasattr(predictions, '__len__') else predictions}"
                    )
            except Exception as e:
        ...
        if results.get("status") == "completed":
            ...
            for symbol, feat_data in features.items():
                if hasattr(feat_data, "shape"):
                    print(
                        f"     {symbol}: {feat_data.shape[1]} features, {feat_data.shape[0]} samples"
                    )
            ...
            print(
                f"   Models trained: {sum(len(task_models) for task_models in models.values())}"
            )
            ...
            print(
                f"   Models deployed: {sum(len(task_deployments) for task_deployments in deployed_models.values())}"
            )
...
async def run_integration_demo(config):
    ...
    try:
        ...
        demo_results = await demonstrate_ml_integration(
            config.symbols[:2]
        )  # Use first 2 symbols
        ...
        for symbol, analysis in agent_analyses.items():
            if "error" not in analysis:
                ...
                if recommendations:
                    ...
                    if reasoning:
                        print(
                            f"     Reasoning: {reasoning[0] if reasoning else 'None provided'}"
                        )

                # Show combined insights
                combined_insights = analysis.get("combined_insights", {})
                if combined_insights:
                    consensus = combined_insights.get(
                        "price_direction_consensus", "unknown"
                    )
        ...
        for symbol, chart in agent_charts.items():
            if "error" not in chart:
                ...
                print(
                    f"   {symbol}: {chart_type} chart with {len(prices)} price points"
                )
...
def print_demo_summary(args, pipeline_success, integration_success):
    ...
    print("\n🏆 Results:")
    print("   Pipeline components demo: ✅ Completed")
    print(f"   Full ML pipeline: {'✅ Success' if pipeline_success else '❌ Failed'}")
    print(
        f"   ML integration demo: {'✅ Success' if integration_success else '❌ Failed'}"
    )
    ...
    print("\n🚀 Next Steps:")
    if pipeline_success:
        print("   • Explore the generated model files and results")
        print("   • Integrate ML predictions into your trading strategy")
        print("   • Set up real-time model monitoring and retraining")
        if args.enable_deployment:
            print("   • Access the inference API at http://localhost:8080")
    else:
        print("   • Check the logs for error details")
        print("   • Try with different symbols or shorter time period")
        print("   • Use --skip-training for a faster demo")

    print("\n📚 Documentation:")
    print("   • Feature Engineering: tradingagents/ml/feature_engineering.py")
    print("   • Model Training: tradingagents/ml/models.py")
    print("   • Validation Framework: tradingagents/ml/validation.py")
    print("   • Deployment Pipeline: tradingagents/ml/deployment.py")
    print("   • Integration: tradingagents/ml/integration.py")

...
class HistoryService:
    ...
    def __init__(self, data_dir: str = None):
        ...
        self.results_dir = (
            current_dir / "reports" if (current_dir / "reports").exists() else None
        )
    ...
    def get_analysis_detail(self, analysis_id: str) -> dict | None:
        """获取分析详情
                ...
                try:
                    with open(file_path, encoding="utf-8") as f:
                        ...
                        if (
                            ...
                            or str(file_path.stem) == analysis_id
                        ):
                            return data
                except Exception as e:
    ...
    def _parse_multi_model_file(self, file_path: Path) -> dict | None:
        """解析多模型协作结果文件"""
        try:
            with open(file_path, encoding="utf-8") as f:
                data = json.load(f)
    ...
    def _parse_report_file(self, file_path: Path) -> dict | None:
        """解析报告文件"""
        try:
            with open(file_path, encoding="utf-8") as f:
                data = json.load(f)

...
# 导入环境变量编辑器
from tradingagents.config.env_editor import (  # noqa: E402
...
from tradingagents.config.env_metadata import (  # noqa: E402
    ENV_FIELDS,
    GROUP_HELP,
    GROUP_ORDER,
    get_field_help,
...
def render_model_config():
    ...
    with col1:
        new_provider = st.selectbox(
            "供应商",
            ["deepseek", "google", "openrouter", "siliconflow", "other"],
            key="new_provider",
        )
...
def render_pricing_config():
    ...
    with col1:
        new_provider = st.text_input(
            "供应商", placeholder="例如: deepseek, google", key="new_pricing_provider"
        )
        new_model_name = st.text_input(
            "模型名称",
...
def render_usage_statistics():
    ...
    # 显示页面迁移提示
    st.info(
        "💡 **完整的Token使用统计功能已迁移至 📈 历史记录 > 统计分析 > 💰 Token使用统计**"
    )
...
def render_system_settings():
    ...
    with col1:
        ...
        default_provider = st.selectbox(
            "默认供应商", providers, index=idx, key="settings_default_provider"
        )
...
def render_env_status():
    ...
    try:
        from tradingagents.config.env_editor import get_effective_env_value, read_env

        env_path = project_root / ".env"
        _raw, env_kv = read_env(env_path)
        # 读取并解析 DEMO_MODE
        demo_mode_effective = (
            get_effective_env_value("DEMO_MODE", env_kv.get("DEMO_MODE", "")) or "false"
        )
        demo_mode_on = str(demo_mode_effective).strip().lower() in {
            "true",
            "1",
            "yes",
            "on",
        ...
        # 读取 DEMO_DATA_FILE
        demo_file_effective = get_effective_env_value(
            "DEMO_DATA_FILE", env_kv.get("DEMO_DATA_FILE", "")
        )
        ...
        with api_col1:
            ...
            for provider, configured in env_status["api_keys"].items():
                if provider in ["deepseek", "google"]:
                    ...
                    provider_name = {"deepseek": "DeepSeek", "google": "Google AI"}.get(
                        provider, provider
                    )
...
def render_env_editor():
    ...
    with col4:
        # 显示待保存的变更数量
        changes_count = len(st.session_state.env_changes) + len(
            st.session_state.env_to_remove
        )
        if changes_count > 0:
                ...
                with col1:
                    ...
                    if field_type == "secret":
                        ...
                        if new_value and new_value != display_value:
                            is_valid, err = validate_env_value(
                                key, new_value, field_type
                            )
                            if not is_valid:
                        ...
                        try:
                            current_float = (
                                float(display_value) if display_value else 0.0
                            )
                        except ValueError:
                            current_float = 0.0
                        ...
                        # 反解英文值
                        new_selection = (
                            options[display_options.index(new_display)]
                            if new_display in display_options
                            else options[0]
                        )

                        if new_selection != display_value:
                            is_valid, err = validate_env_value(
                                key, new_selection, field_type
                            )
                            if not is_valid:
                                st.error(err or "该取值不合法，请检查格式或范围")
                            else:
    ...
    with st.expander("➕ 添加新字段", expanded=False):
        ...
        with col1:
            new_key = st.text_input(
                "字段名", placeholder="NEW_VARIABLE", key="new_env_key"
            )
        with col2:
        ...
        with col3:
            if st.button("添加", key="add_new_env_field"):
                if new_key and new_value:
                    if (
                        new_key.upper() == new_key
                        and new_key.replace("_", "").isalnum()
                    ):

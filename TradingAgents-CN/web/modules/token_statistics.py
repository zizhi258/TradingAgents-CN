...
def render_token_statistics():
    """渲染Token统计页面（已弃用，重定向到历史记录页面）"""
    # 应用隐藏Deploy按钮的CSS样式
    apply_hide_deploy_button_css()
    
    # 显示弃用警告
    st.warning(
        "⚠️ **页面已迁移** - 本页面已合并至 📈 历史记录 > 统计分析 > 💰 Token使用统计。"
        "建议从顶部导航进入该页面以获得最新功能。",
        icon="⚠️"
    )
    ...
    with col2:
        if st.button("🔗 前往历史记录页面", type="primary", use_container_width=True):
    ...
    st.markdown("---")
    st.markdown("### 💰 Token使用统计（兼容模式）")
    st.info("以下内容将保持可用，但推荐使用新的统一界面。")
    
    # 调用历史记录页面的Token统计功能（简化版本）
    try:
        from components.history_manager import _render_token_statistics
        _render_token_statistics()
    except ImportError:
        # 如果无法导入新版本，显示原有功能
        st.error("❌ 无法加载新版Token统计功能，请检查系统配置")
...
def _render_legacy_token_statistics():
    ...
    with st.sidebar:
        ...
        # 刷新按钮
        if st.button("🔄 刷新数据", use_container_width=True, key="legacy_token_refresh"):
            st.rerun()
        
        # 导出数据按钮
        if st.button("📥 导出统计数据", use_container_width=True, key="legacy_token_export"):

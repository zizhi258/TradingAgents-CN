...
def _render_legacy_token_statistics():
    ...
    with st.sidebar:
        ...
        # 刷新按钮
        if st.button(
            "🔄 刷新数据", use_container_width=True, key="legacy_token_refresh"
        ):
            st.rerun()

        # 导出数据按钮
        if st.button(
            "📥 导出统计数据", use_container_width=True, key="legacy_token_export"
        ):

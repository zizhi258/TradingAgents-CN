...
def create_agent_flow_diagram(
    flow_config: dict[str, Any], agents_config: dict[str, Any]
) -> None:
    """创建智能体信息流图"""
        ...
        for tier, position in tier_positions.items():
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="markers+text",
                    marker=dict(
                        size=25, color=colors, line=dict(width=2, color="white")
                    ),
                    text=text_labels,
        ...
        for i in range(len(tier_positions) - 1):
            ...
            if current_agents and next_agents:
                ...
                for curr_agent in current_agents:
                    curr_idx = [
                        n["id"] for n in nodes if n["tier"] == current_tier
                    ].index(curr_agent["id"])
                    curr_x = curr_idx * 2 - (len(current_agents) - 1)

                    for next_agent in next_agents:
                        next_idx = [
                            n["id"] for n in nodes if n["tier"] == next_tier
                        ].index(next_agent["id"])
                        ...
                        fig.add_trace(
                            go.Scatter(
                                x=[curr_x, next_x],
                                y=[
                                    tier_positions[current_tier]["y"],
                                    tier_positions[next_tier]["y"],
                                ],
                                mode="lines",
        ...
        table_data.append(
            {
                ...
                "描述": (
                    agent_info.get("description", "")[:80] + "..."
                    if len(agent_info.get("description", "")) > 80
                    else agent_info.get("description", "")
                ),
                "状态": (
                    "✅ 启用"
                    if is_enabled
                    else (
                        "🟡 可选" if agent_info.get("is_optional", False) else "❌ 禁用"
                    )
                ),
...
def render_system_status_panel() -> None:
    ...
    with col2:
        # 系统健康度
        health_score = (
            (available_agents / total_agents) * 100 if total_agents > 0 else 0
        )
        health_status = (
            "优秀"
            if health_score >= 90
            else ("良好" if health_score >= 75 else "需要关注")
        )

        st.metric(label="系统健康度", value=f"{health_score:.0f}%", delta=health_status)
...
def render_charting_artist_status() -> None:
    ...
    if is_enabled:
        ...
        if charts_dir.exists():
            chart_count = len(list(charts_dir.glob("*.html"))) + len(
                list(charts_dir.glob("*.json"))
            )
...
def get_system_status() -> list[dict[str, str]]:
    ...
    # 检查基本组件
    status_list.append(
        {"component": "Web界面", "status": "正常", "message": "Streamlit服务运行正常"}
    )
    ...
    if data_dir.exists():
        status_list.append(
            {"component": "数据目录", "status": "正常", "message": "数据目录可访问"}
        )
    else:
        status_list.append(
            {"component": "数据目录", "status": "异常", "message": "数据目录不存在"}
        )
    ...
    if config_file.exists():
        status_list.append(
            {"component": "智能体配置", "status": "正常", "message": "角色配置文件存在"}
        )
    else:
        status_list.append(
            {"component": "智能体配置", "status": "警告", "message": "角色配置文件缺失"}
        )
...
def load_agent_roles_config() -> dict[str, Any] | None:
    """加载智能体角色配置"""

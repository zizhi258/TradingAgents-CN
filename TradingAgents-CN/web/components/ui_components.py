...
def render_button_radio(
    options: list[str], key: str, default_value: Any = None, horizontal: bool = True
):
    """
    ...
    if key not in st.session_state:
        st.session_state[key] = (
            default_value if default_value is not None else options[0]
        )
    ...
    if os.getenv("DEBUG_MODE", "false").lower() == "true":
        st.markdown(
            f"<small>调试信息: key=`{key}`, value=`{st.session_state.get(key)}`</small>",

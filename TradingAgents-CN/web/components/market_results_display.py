...
class AdvancedStockRankingsDisplay:
    ...
    def _render_control_panel(self, key_suffix: str):
        ...
        with col2:
            order = st.selectbox(
                "排序方向", options=["降序", "升序"], key=f"ranking_order_{key_suffix}"
            )
    ...
    def _render_rankings_table(self, rankings_data: list[dict], key_suffix: str):
        ...
        with col_p1:
            page_size = st.slider(
                "每页数量", 10, 200, 50, 10, key=f"rank_page_size_{key_suffix}"
            )
        total_rows = len(df)
        ...
        st.markdown("#### 📤 导出与操作")
        sel_idx = list(
            getattr(getattr(selected_rows, "selection", None), "rows", []) or []
        )
        ...
        def df_to_excel_bytes(d: pd.DataFrame) -> bytes | None:
            try:
                import io as _io
        ...
        with col_e1:
            st.download_button(
                "下载选中CSV",
                data=(
                    df_to_csv_bytes(selected_subset)
                    if not selected_subset.empty
                    else b""
                ),
                file_name="selected_rankings.csv",
                mime="text/csv",
                disabled=selected_subset.empty,
            )
        with col_e2:
            xbytes = (
                df_to_excel_bytes(selected_subset)
                if not selected_subset.empty
                else None
            )
            st.download_button(
                "下载选中Excel",
                data=xbytes or b"",
                file_name="selected_rankings.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                disabled=(xbytes is None),
            )
        with col_e3:
            st.download_button(
                "下载本页CSV",
                data=df_to_csv_bytes(page_df),
            ...
            try:
                ...
                for _, row in selected_subset.iterrows():
                    ...
                    if recs:
                        ...
                        # 兼容trade_date/日期字段
                        date_col = (
                            "trade_date" if "trade_date" in df_line.columns else "date"
                        )
                        price_col = (
                            "close"
                            if "close" in df_line.columns
                            else df_line.columns[-1]
                        )
                        df_line[date_col] = _pd.to_datetime(df_line[date_col])
                        df_line["symbol"] = code
                        df_line.rename(columns={price_col: "close"}, inplace=True)
                        frames.append(
                            df_line[[date_col, "close", "symbol"]].rename(
    ...
    def _render_score_distribution(self, rankings_data: list[dict], key_suffix: str):
        ...
        # 综合评分分布
        fig_hist.add_trace(
            go.Histogram(x=df["total_score"], name="综合评分", nbinsx=20), row=1, col=1
        )
        ...
        fig_hist.update_layout(
            height=600, title_text="评分分布综合分析", showlegend=False
        )
        ...
        with col2:
            ...
            st.metric(
                "中等股票数(60-80)",
                len(df[(df["total_score"] >= 60) & (df["total_score"] < 80)]),
            )

        with col3:
            st.metric(
                "技术领先股票数",
                len(df[df["technical_score"] > df["fundamental_score"]]),
            )
            st.metric(
                "基本面优势股票数",
                len(df[df["fundamental_score"] > df["technical_score"]]),
            )

    def _render_recommendation_analysis(
        self, rankings_data: list[dict], key_suffix: str
    ):
        """渲染推荐分析"""
        ...
        fig_pie.update_layout(
            title="投资建议分布",
            annotations=[
                dict(text="投资建议", x=0.5, y=0.5, font_size=16, showarrow=False)
            ],
        )
        ...
        if not buy_stocks.empty:
            ...
            buy_display["潜在收益"] = (
                (buy_display["target_price"] - buy_display["current_price"])
                / buy_display["current_price"]
                * 100
            ).round(2)
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "潜在收益": st.column_config.NumberColumn(
                        "潜在收益(%)", format="%.2f"
                    ),
                },
                use_container_width=True,
    ...
    def _render_price_analysis(self, rankings_data: list[dict], key_suffix: str):
        ...
        fig_price.update_layout(
            height=700, title_text="价格与估值综合分析", showlegend=False
        )
        ...
        with col3:
            ...
            st.metric(
                "有上涨空间股票", len(df[df["target_price"] > df["current_price"]])
            )
    ...
    def _prepare_rankings_dataframe(
        self, rankings_data: list[dict], controls: dict
    ) -> pd.DataFrame:
        """准备排名数据框"""
    ...
    def _get_recommendation_column_config(self):
        ...
        return st.column_config.TextColumn(
            "建议", width="small", help="投资建议：买入、持有、关注、卖出"
        )

    def _show_selected_stocks_details(
        self, df: pd.DataFrame, selected_rows: list[int], key_suffix: str
    ):
        """显示选中股票的详细信息"""
        ...
        for _, stock in selected_df.iterrows():
            with st.expander(
                f"📊 {stock.get('股票名称', '')} ({stock.get('股票代码', '')})"
            ):
                ...
                with detail_col3:
                    ...
                    st.metric(
                        "涨跌幅", f"{change_pct:+.2f}%", delta=f"{change_pct:+.2f}%"
                    )
    ...
    def _render_sector_detailed_analysis(self, sectors_data: dict, key_suffix: str):
        ...
        if selected_sector and selected_sector in sectors_data:
            ...
            with col1:
                ...
                st.metric(
                    "板块涨跌幅", f"{change_pct:+.2f}%", delta=f"{change_pct:+.2f}%"
                )
            ...
            if "recommended_stocks" in sector_data:
                ...
                if recommended_stocks:
                    ...
                    if not rec_df.empty:
                        ...
                        available_cols = [
                            col for col in display_cols if col in rec_df.columns
                        ]
...
class MarketBreadthGauge:
    ...
    def render(self, breadth_data: dict[str, Any]) -> None:
        ...
        with col1:
            self._render_gauge_chart(
                "市场情绪指数", breadth_data.get("sentiment_index", 50)
            )

        with col2:
            self._render_gauge_chart(
                "市场强度指数", breadth_data.get("market_strength", 50)
            )
    ...
    def _render_market_status_summary(self, breadth_data: dict):
        ...
        if status_color == "success":
            st.success(
                f"{status_emoji} **市场状态: {status_text}** (得分: {overall_score:.1f})"
            )
        ...
        if (
            breadth_data.get("limit_up_count", 0)
            > breadth_data.get("limit_down_count", 0) * 2
        ):
            observations.append("✅ 涨停股票远多于跌停，市场情绪乐观")
        elif (
            breadth_data.get("limit_down_count", 0)
            > breadth_data.get("limit_up_count", 0) * 2
        ):
...
def create_comparison_chart(
    data1: list, data2: list, labels: list[str], title: str, y_title: str
) -> go.Figure:
    """创建对比图表"""
    ...
    fig.add_trace(
        go.Bar(
            name=labels[0], x=list(range(len(data1))), y=data1, marker_color="lightblue"
        )
    )
    ...
    fig.update_layout(
        title=title, xaxis_title="类别", yaxis_title=y_title, barmode="group"
    )

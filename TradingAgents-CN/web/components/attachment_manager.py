    ...
    if uploaded_files:
        ...
        with col2:
            auto_rename = st.checkbox(
                "自动重命名", value=True, help="为文件添加时间戳前缀避免重名"
            )

        # 文件描述
        file_description = st.text_area(
            "文件描述（可选）", placeholder="描述这些文件的用途和内容...", height=100
        )
        ...
        if st.button("📤 开始上传", type="primary"):
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                try:
                    ...
                    status_text.text(
                        f"正在上传: {uploaded_file.name} ({i+1}/{total_files})"
                    )
...
def render_file_list(file_manager: FileManager):
    ...
    with col3:
        items_per_page = st.selectbox("每页显示", [10, 20, 50, 100], index=1)
    ...
    if page_files:
            ...
            df_data.append(
                {
                    ...
                    "创建时间": (
                        ...
                        else str(file_info.get("created_at", ""))
                    ),
                    "文件ID": file_info["id"],
                }
            )
        ...
        with col4:
            if st.button("🗑️ 删除选中文件"):
                if hasattr(selected_rows, "selection") and selected_rows.selection.get(
                    "rows"
                ):
        ...
        with col7:
            if st.button("📧 发送到订阅邮箱"):
                if hasattr(selected_rows, "selection") and selected_rows.selection.get(
                    ...
                    try:
                        from tradingagents.services.mailer.email_sender import (
                            EmailSender,
                        )
                        ...
                        for idx in selected_rows.selection["rows"]:
                            ...
                            subs = [
                                s
                                for s in sm.get_active_subscriptions(
                            ...
                            analysis_result = {
                                "analysis_date": (
                                    fi.get("created_at").strftime("%Y-%m-%d")
                                    if fi.get("created_at")
                                    else ""
                                ),
                                "decision": {
                                    "action": "报告",
                            ...
                            if es.send_analysis_report(
                                recipients=recipients,
                                stock_symbol=symbol or "REPORT",
...
def render_file_settings(file_manager: FileManager):
    ...
    with col1:
        ...
        st.checkbox(
            "自动清理临时文件", value=True, help="自动删除过期的临时文件"
        )
...
def _show_file_details(file_info: dict):
    ...
    with st.expander(f"📄 文件详情: {file_info['filename']}", expanded=True):
        ...
        with col1:
            ...
            st.write(
                f"**原始名称**: {file_info.get('original_name', file_info['filename'])}"
            )

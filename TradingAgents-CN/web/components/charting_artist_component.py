...
from .chart_batch_operations import (
    ...
    render_all_charts_interface,
    render_batch_operations_interface,
    render_cleanup_interface,
    render_usage_statistics_interface,
    share_chart_enhanced,
    show_all_charts_list,
    show_batch_operations_dialog,
...
class ChartingArtistAPIClient:
    ...
    def get_chart_by_id(self, chart_id: str) -> dict[str, Any]:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/charts/{chart_id}", timeout=30
            )
...
def render_visualization_section(
    analysis_results: dict[str, Any], symbol: str = None, analysis_id: str = None
) -> None:
    """
...
def render_charting_artist_interface(
    analysis_results: dict[str, Any], symbol: str = None, analysis_id: str = None
) -> None:
    """渲染ChartingArtist主界面"""
    ...
    with col2:
        ...
        st.markdown(
            f"**API状态:** :{status_color}[{'在线' if api_available else '离线'}]"
        )
...
def execute_chart_generation_enhanced(
    ...
    with st.expander("⚙️ 生成配置", expanded=True):
        ...
        with col2:
            st.markdown("**图表配置**")
            theme = st.selectbox(
                "主题", ["plotly_dark", "plotly_white", "ggplot2", "seaborn"]
            )
    ...
    try:
        ...
        if api_available:
            ...
            if "error" not in result:
                ...
                st.success(
                    f"🎯 API成功生成 {result.get('total_charts', len(chart_types))} 个图表"
                )

                # 显示生成详情
                if result.get("charts"):
                    st.info(
                        f"生成的图表类型: {', '.join([c.get('chart_type', '未知') for c in result['charts']])}"
                    )

            else:
                # API失败，回退到本地生成
                status_text.text("🔄 API失败，使用本地生成...")
                local_charts = generate_local_charts_enhanced(
                    analysis_results, symbol, chart_types, generation_config
                )
                handle_local_generation_result(
                    local_charts, analysis_results, progress_bar, status_text
                )
        else:
            # 直接使用本地生成
...
def handle_local_generation_result(
...
def prepare_enhanced_data_sources(
...
def generate_local_charts_enhanced(
    analysis_results: dict[str, Any],
    symbol: str,
...
def display_existing_charts_enhanced(
    analysis_results: dict[str, Any],
    symbol: str = None,
    analysis_id: str = None,
    api_client: ChartingArtistAPIClient = None,
) -> None:
    """增强版现有图表显示"""
...
def display_charts_with_management(
    charts: list[dict[str, Any]], api_client: ChartingArtistAPIClient = None
) -> None:
    """显示图表并提供管理功能"""
    ...
    with col3:
        generation_method = (
            charts[0].get("metadata", {}).get("generation_method", "未知")
        )
...
def display_chart_group_enhanced(
    charts: list[dict[str, Any]], api_client: ChartingArtistAPIClient = None
) -> None:
    """增强版图表组显示"""
        ...
        try:
            ...
            with col2:
                ...
                if st.button(
                    "⬇️",
                    help="下载图表",
                    key=f"download_enhanced_{chart.get('chart_id', i)}",
                ):
            ...
            with col3:
                ...
                if st.button(
                    "🔗",
                    help="分享图表",
                    key=f"share_enhanced_{chart.get('chart_id', i)}",
                ):
            ...
            with col4:
                ...
                if st.button(
                    "🗑️",
                    help="删除图表",
                    key=f"delete_enhanced_{chart.get('chart_id', i)}",
                ):
            ...
            with col5:
                ...
                if st.button(
                    "📚",
                    help="保存到图书馆",
                    key=f"save_lib_enh_{chart.get('chart_id', i)}",
                ):
...
def render_chart_management_enhanced(
    api_client: ChartingArtistAPIClient, api_available: bool
) -> None:
    """增强版图表管理面板"""
...
def display_existing_charts(
    analysis_results: dict[str, Any], symbol: str = None
) -> None:
    """显示现有的图表"""
    ...
    if charts_generated:
        ...
        # 创建标签页显示不同类型的图表
        chart_types = list(
            set([chart.get("chart_type", "unknown") for chart in charts_generated])
        )
        ...
        if len(chart_types) > 1:
            ...
            # 单一图表类型直接显示
            display_charts_by_type(
                charts_generated, chart_types[0] if chart_types else None
            )
...
def display_charts_by_type(
    charts: list[dict[str, Any]], chart_type: str = None
) -> None:
    """按类型显示图表"""

    filtered_charts = (
        [c for c in charts if c.get("chart_type") == chart_type]
        if chart_type
        else charts
    )
    ...
    for chart in filtered_charts:
        try:
            ...
            if chart_path and Path(chart_path).exists():
                ...
                with col1:
                    if st.button(
                        "⬇️ 下载", key=f"download_{chart.get('chart_id', 'unknown')}"
                    ):
                        download_chart(chart_path)
                with col2:
                    if st.button(
                        "🔗 分享", key=f"share_{chart.get('chart_id', 'unknown')}"
                    ):
                        share_chart(chart)
...
def generate_charts_for_results(
    analysis_results: dict[str, Any], symbol: str = None
) -> None:
    """为分析结果生成图表"""
    ...
    try:
        with st.spinner("🎨 正在生成图表..."):
            ...
            request_data = {
                ...
                "config": {"theme": "plotly_dark", "interactive": True},
            }
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # 显示成功信息
                    st.success(
                        f"🎯 成功生成 {viz_results.get('total_charts', 0)} 个图表"
                    )
...
def create_simple_price_chart(symbol: str) -> str | None:
    """创建简单的价格图表"""
    try:
        import uuid
        ...
        chart_file = (
            charts_dir / f"{symbol}_line_chart_{uuid.uuid4().hex[:8]}_local.html"
        )
...
def create_simple_financial_chart(symbol: str) -> str | None:
    """创建简单的财务图表"""
    try:
        import uuid
        ...
        chart_file = (
            charts_dir / f"{symbol}_bar_chart_{uuid.uuid4().hex[:8]}_local.html"
        )
        fig.write_html(str(chart_file))
...
def display_chart_settings() -> None:
    ...
    st.selectbox("主题", ["plotly_dark", "plotly_white", "ggplot2", "seaborn"])
    st.number_input("宽度", min_value=400, max_value=1200, value=800)
    st.number_input("高度", min_value=300, max_value=800, value=600)
    st.checkbox("交互式图表", value=True)

...
def check_api_keys():
    ...
    gemini_api_compat_key = os.getenv("GEMINI_API_COMPAT_API_KEY") or os.getenv(
        "OPENAI_API_KEY"
    )
    ...
    details = {
        ...
        "GEMINI_API_COMPAT_API_KEY": {
            ...
            "display": (
                f"{gemini_api_compat_key[:12]}..."
                if gemini_api_compat_key
                else "未配置"
            ),
            "required": False,
        ...
        "GOOGLE_FAMILY_API_KEY": {
            ...
            "display": (
                f"{google_key[:12]}... (via {google_key_name})"
                if google_key
                else "未配置"
            ),
            "required": False,

...
class MarketScanConfig:
    ...
    time_range: str  # 时间范围
    custom_filters: dict | None = None  # 自定义筛选条件
    analysis_focus: dict | None = None  # 分析重点
    ai_model_config: dict | None = None  # AI模型配置
    advanced_options: dict | None = None  # 高级选项
...
class ScanProgress:
    ...
    stats: dict
    latest_message: str
    preview_results: dict | None = None
...
class MarketAnalysisAPIClient:
    ...
    def create_scan(self, config: MarketScanConfig) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"创建市场扫描请求: {config.market_type}, 深度: {config.scan_depth}, 预算: {config.budget_limit}"
            )
    ...
    def get_scan_status(self, scan_id: str) -> ScanProgress:
        ...
        try:
            url = (
                f"{self.base_url}{self.endpoints['scan_status'].format(run_id=scan_id)}"
            )
    ...
    def get_scan_rankings(
        ...
        try:
            ...
            params = {"sort_by": sort_by, "order": order, "limit": limit}
    ...
    def list_stocks(self, page: int = 1, page_size: int = 50) -> dict[str, Any]:
        ...
        r = self.session.get(
            url, params={"page": page, "page_size": page_size}, timeout=self.timeout
        )
        r.raise_for_status()
        return r.json()
    ...
    def get_daily(
        self, code: str, start_date: str = None, end_date: str = None, adj: str = "qfq"
    ) -> dict[str, Any]:
    ...
    def filter_group(
        self, group: str, page: int = 1, page_size: int = 100
    ) -> dict[str, Any]:
    ...
    def export_results(
        self, scan_id: str, export_format: str = "excel", options: dict | None = None
    ) -> bytes | str:
        """
        导出扫描结果
        ...
        try:
            url = (
                f"{self.base_url}{self.endpoints['scan_export'].format(run_id=scan_id)}"
            )
    ...
    def pause_scan(self, scan_id: str) -> bool:
        ...
        try:
            url = (
                f"{self.base_url}{self.endpoints['pause_scan'].format(run_id=scan_id)}"
            )
            ...
            if success:
                ...
                logger.warning(
                    f"扫描暂停失败: {scan_id}, 原因: {result.get('message', 'unknown')}"
                )
    ...
    def resume_scan(self, scan_id: str) -> bool:
        ...
        try:
            url = (
                f"{self.base_url}{self.endpoints['resume_scan'].format(run_id=scan_id)}"
            )
            ...
            if success:
                ...
                logger.warning(
                    f"扫描继续失败: {scan_id}, 原因: {result.get('message', 'unknown')}"
                )
    ...
    def cancel_scan(self, scan_id: str) -> bool:
        ...
        try:
            url = (
                f"{self.base_url}{self.endpoints['cancel_scan'].format(run_id=scan_id)}"
            )
            ...
            if success:
                ...
                logger.warning(
                    f"扫描取消失败: {scan_id}, 原因: {result.get('message', 'unknown')}"
                )
...
class MockMarketAnalysisAPIClient(MarketAnalysisAPIClient):
    ...
    def get_scan_status(self, scan_id: str) -> ScanProgress:
        ...
        stages = [
            {"name": "数据准备", "completed": progress > 10, "current": progress <= 10},
            {
                ...
                "current": 10 < progress <= 30,
            },
            {
                "name": "技术分析",
                "completed": progress > 60,
                "current": 30 < progress <= 60,
            },
            {
                "name": "基本面分析",
                "completed": progress > 80,
                "current": 60 < progress <= 80,
            },
            {
                "name": "生成报告",
                "completed": progress > 95,
                "current": 80 < progress <= 95,
            },
        ]
        ...
        stats = {
            ...
            "recommended_stocks": (
                int(progress * config.stock_limit / 500) if progress >= 100 else 0
            ),
        }
        ...
        return ScanProgress(
            ...
            preview_results=(
                self._generate_preview_results(progress) if progress < 100 else None
            ),
            estimated_completion=(
                f"{max(1, int((100 - progress) / 10))}分钟"
                if progress < 100
                else "已完成"
            ),
        )
    ...
    def resume_scan(self, scan_id: str) -> bool:
        ...
        if (
            scan_id in self.mock_data
            and self.mock_data[scan_id]["status"] == ScanStatus.PAUSED
        ):
...
class LocalMarketAnalysisClient(MarketAnalysisAPIClient):
    ...
    def __init__(self):
        # 不调用父类初始化，避免HTTP会话
        from tradingagents.core.market_scan_orchestrator import (
            MarketScanOrchestrator,
            OrchestratorConfig,
        )
    ...
    def create_scan(self, config: MarketScanConfig) -> dict[str, Any]:
        ...
        def _progress_cb(msg: str, cur: int, total: int):
            try:
                ...
                st["stages"] = [
                    {
                        "name": "准备环境",
                        "completed": st["progress"] > 5,
                        "current": st["progress"] <= 5,
                    },
                    {
                        "name": "股票池",
                        "completed": st["progress"] > 10,
                        "current": 5 < st["progress"] <= 10,
                    },
                    {
                        "name": "并发评估",
                        "completed": st["progress"] > 85,
                        "current": 10 < st["progress"] <= 85,
                    },
                    {
                        "name": "聚合统计",
                        "completed": st["progress"] > 95,
                        "current": 85 < st["progress"] <= 95,
                    },
                    {
                        "name": "生成摘要",
                        "completed": st["progress"] >= 100,
                        "current": 95 < st["progress"] < 100,
                    },
                ]
        ...
        def _runner():
            ...
            try:
                ...
                st["stats"]["scan_duration"] = (
                    stats.get("scan_duration") or f"{int(time.time()-t0)}秒"
                )

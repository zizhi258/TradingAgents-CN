...
from utils.market_api_client import (
    MarketScanConfig,
    ScanStatus,
    get_api_client,
)
...
class MarketSessionManager:
    ...
    def __init__(
        self, data_dir: str = "./data/market_sessions", use_mock_api: bool = True
    ):
        """
        初始化会话管理器
    ...
    def get_session_progress(self, scan_id: str) -> dict[str, Any] | None:
        """
        获取会话进度信息
        ...
        try:
            ...
            if "扫描不存在" in msg or "not found" in msg.lower():
                ...
                return {"scan_id": scan_id, "status": "not_found", "error_message": msg}

            return {"scan_id": scan_id, "status": "error", "error_message": msg}

    def get_session_results(self, scan_id: str) -> dict[str, Any] | None:
        """
        获取会话的完整结果
    ...
    def export_session_data(
        self, scan_id: str, format_type: str = "json"
    ) -> str | None:
        """
        导出会话数据
    ...
    def _start_progress_update_thread(self, scan_id: str):
        ...
        thread = threading.Thread(
            target=self._progress_update_worker, args=(scan_id, stop_flag), daemon=True
        )
    ...
    def _get_last_cleanup_time(self) -> str | None:
        """获取最后清理时间"""
...
def init_market_session_state():
    ...
    if "market_session_manager" not in st.session_state:
        ...
        use_mock = os.getenv("MARKET_SCAN_USE_MOCK", "true").lower() in [
            "1",
            "true",
            "yes",
            "on",
        ]
        st.session_state.market_session_manager = MarketSessionManager(
...
if __name__ == "__main__":
    ...
    with MarketSessionManager(use_mock_api=True) as manager:
        ...
        for i in range(10):
            ...
            if progress:
                print(
                    f"进度: {progress['overall_progress']}% - {progress['latest_message']}"
                )
                ...
                if progress["status"] == "completed":
                    ...
                    print(
                        f"扫描{progress['status']}: {progress.get('error_message', '')}"
                    )
                    break

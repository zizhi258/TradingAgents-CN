...
# 导入统一日志系统
from tradingagents.utils.logging_init import setup_web_logging  # noqa: E402
...
def run_stock_analysis(
    ...
    llm_quick_model: str = None,
    llm_deep_model: str = None,
    ...
    try:
        ...
        # 预获取股票数据（默认30天历史数据）
        # 按研究深度收敛数据窗口，降低I/O与前置耗时
        try:
            _depth_days = {1: 14, 2: 21, 3: 30, 4: 60, 5: 90}
            _period_days = _depth_days.get(
                int(research_depth) if research_depth is not None else 3, 30
            )
        except Exception:
            _period_days = 30
    ...
    if TOKEN_TRACKING_ENABLED:
        ...
        estimated_cost = token_tracker.estimate_cost(
            provider_for_cost, model_for_cost, estimated_input, estimated_output
        )
        ...
        if (
            ...
            and max_budget > 0
            and estimated_cost > max_budget
        ):
            update_progress(
                f"⚠️ 预估成本高于预算上限¥{max_budget:.2f}，将尝试使用更便宜的快速模型/降级策略"
            )
    ...
    try:
        ...
        if research_depth == 1:  # 1级 - 快速分析
            ...
            if llm_provider == "deepseek":
                config["quick_think_llm"] = (
                    config.get("quick_think_llm", "deepseek-chat") or "deepseek-chat"
                )
            ...
            if llm_provider == "deepseek":
                ...
                config["quick_think_llm"] = (
                    config.get("quick_think_llm", "gemini-2.5-pro") or "gemini-2.5-pro"
                )
                config["deep_think_llm"] = (
                    config.get("deep_think_llm", "gemini-2.5-pro") or "gemini-2.5-pro"
                )
        ...
        if TOKEN_TRACKING_ENABLED:
            ...
            actual_input_tokens = len(analysts) * (
                ...
                else 2500 if research_depth == "标准" else 4000
            )
            actual_output_tokens = len(analysts) * (
                800
                if research_depth == "快速"
                else 1200 if research_depth == "标准" else 2000
            )
...
def format_analysis_results(results):
    ...
    try:
        if "success" not in results and isinstance(results.get("results"), dict):
            ...
            # 组合最终返回结构
            return {
                "stock_symbol": results.get("analysis_data", {}).get("stock_symbol")
                or "N/A",
                "decision": decision,
                "state": flat_state,
                "success": True,
                ...
                "research_depth": results.get("analysis_data", {}).get(
                    "research_depth", 3
                ),
                "llm_provider": "multi",
                "llm_model": "multi",

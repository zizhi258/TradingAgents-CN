...
def build_markdown_sections_from_demo(data: dict[str, Any]) -> dict[str, str]:
    ...
    if last:
        ...
        market_md.append(
            f"- 成交量: {last.get('volume','N/A')}  成交额: {last.get('amount','N/A')}"
        )
    if tech:
        ...
        macd = tech.get("macd", {})
        market_md.append(
            f"  - RSI(14)={rsi14}  MACD: dif={macd.get('dif')} dea={macd.get('dea')} hist={macd.get('hist')}"
        )
    market_report = "\n".join(market_md)
    ...
    if fundamentals:
        ...
        fund_md.append(
            "  - 收入: {0:,} 元  净利润: {1:,} 元".format(
                int(fundamentals.get("revenue", 0) or 0),
                int(fundamentals.get("net_profit", 0) or 0),
            )
        )
        fund_md.append(
    ...
    invest_md = [
        "## 📋 投资建议(演示)",
        "- 策略: 以演示数据生成的参考结论，非投资建议",
        "- 建议: 观察关键均线与RSI拐点，分批操作",
    ]
    invest_report = "\n".join(invest_md)
...
def build_final_article_from_sections(
    name: str, sections: dict[str, str]
) -> tuple[str, dict[str, Any]]:
    content = [
        f"# {name} 多维度演示分析报告",
        sections.get("fundamentals_report", ""),
        sections.get("market_report", ""),
        sections.get("news_report", ""),
        sections.get("risk_assessment", ""),
        sections.get("investment_plan", ""),
    ]
...
def build_multi_model_results_from_demo(
    ...
    # 组装智能体结果（简化文本）
    agents = list(
        selected_agents or ["news_hunter", "fundamental_expert", "risk_manager"]
    )
    ...
    return {
        ...
        "analysis_data": {
            ...
            "analysis_date": meta.get("trade_date")

...
- **多提供商支持**: DeepSeek V3、Google AI、OpenRouter
...
#### 🎨 可视化 API 联调说明（ChartingArtist）
- 统一接口路径: `http://<api-host>:8000/api/charts`（Web 端已默认使用该路径）
- Docker 本地默认：`web` 容器通过内部地址访问 `http://api:8000/api`（compose 已配置 `CHARTING_ARTIST_API_URL`）
- 启用可视化服务（在 API 容器生效）:
  - `CHARTING_ARTIST_ENABLED=true`
  - 共享图表存储目录：`./data:/app/data`（已在 compose 中挂载）
- 常见排查：
  - 404 单图接口：前端应请求 `/api/charts/chart/{chart_id}`，非 `/api/charts/{chart_id}`
  - 健康检查：`GET http://localhost:8000/api/charts/health`
  - 生成图表：`POST http://localhost:8000/api/charts/generate`
...
## 🎨 ChartingArtist API 配置

### API服务部署

**Docker部署（推荐）**：
```bash
# API服务已包含在docker-compose.yml中，一键启动
docker-compose up -d
...
# 或使用gunicorn（生产环境）
gunicorn -w 4 -k uvicorn.workers.UvicornWorker tradingagents.api.main:app --bind 0.0.0.0:8000
```
...
# API服务地址配置
CHARTING_ARTIST_API_URL=http://localhost:8000/api  # 本地部署
# CHARTING_ARTIST_API_URL=http://api:8000/api     # Docker内部通信
...
| 端点 | 方法 | 描述 | 示例 |
|------|------|------|------|
| `/api/charts/generate` | POST | 生成图表 | 支持同步/异步模式 |
| `/api/charts/{analysis_id}` | GET | 获取分析的图表列表 | 分页查询 |
| `/api/charts/chart/{chart_id}` | GET | 获取特定图表 | 返回图表详情 |
| `/api/charts/chart/{chart_id}` | DELETE | 删除图表 | 清理存储空间 |
| `/api/charts/list` | GET | 列出所有图表 | 支持分页 |
| `/api/charts/batch` | POST | 批量操作 | 生成/删除/导出 |

#### 图表类型支持

- 📊 **K线图** (`candlestick`): 完整OHLC数据、技术指标、成交量
- 📈 **折线图** (`line_chart`): 价格趋势、移动平均线
- 📊 **柱状图** (`bar_chart`): 财务指标对比、同行分析
- 🥧 **饼图** (`pie_chart`): 业务构成、资产配置
- 🎯 **散点图** (`scatter_plot`): 风险收益分析、相关性
- 🔥 **热力图** (`heatmap`): 相关性矩阵、行业表现
- 🕸️ **雷达图** (`radar_chart`): 综合评分、多维对比
- ⚡ **仪表盘** (`gauge_chart`): 风险评级、置信度
- 💧 **瀑布图** (`waterfall`): 财务流变化
- 📦 **箱线图** (`box_plot`): 收益率分布、异常值检测

### 前端集成示例

```python
# Web组件中的API调用示例
from web.components.enhanced_visualization_tab import ChartingArtistAPI

# 初始化API客户端
api = ChartingArtistAPI(base_url="http://localhost:8000/api")
...
# 获取图表列表
charts = api.get_charts("analysis_123")

# 删除图表
api.delete_chart("chart_abc123")
```
...
```bash
# 检查前端API配置
grep -r "CHARTING_ARTIST_API_URL" .env
grep -r "/charts/" web/components/

# 验证后端路由
curl -I http://localhost:8000/api/charts/list
```
...
```bash
# 确保图表输出目录权限
mkdir -p data/attachments/charts
chmod 755 data/attachments/charts
```
...
**AI模型**: DeepSeek V3 | Google AI | OpenRouter(60+模型) | OpenAI
...
- **🚀 快速开始**: [docs/overview/quick-start.md](./docs/overview/quick-start.md) - 5分钟快速部署指南

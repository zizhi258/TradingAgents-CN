...
from .smart_routing_engine import SmartRoutingEngine
...
class MultiModelManager:
    ...
    def _initialize_clients(self) -> None:
        ...
        if not self.clients:
            error_details = "; ".join(
                [f"{name}: {error}" for name, error in failed_clients]
            )
        ...
        if failed_clients:
            ...
            logger.warning(
                f"部分客户端初始化失败: {failed_names}，系统将使用可用客户端继续运行"
            )
    ...
    def _initialize_routing_engine(self) -> None:
        ...
        try:
            ...
            self.routing_engine = SmartRoutingEngine(
                database_path=self.database_path, config=routing_config
            )
            ...
            handle_user_friendly_error(
                e, {"action": "routing_engine_initialization"}
            )
    ...
    def select_optimal_model(
        ...
        try:
            ...
            if not candidate_models:
                ...
                logger.warning(
                    f"策略过滤后候选为空，回退到全量可用模型: agent={agent_role}, task={task_type}"
                )

            # 如果存在锁定模型并且可用，直接返回
            if locked_model and locked_model in candidate_models:
                logger.info(
                    f"使用锁定模型: {locked_model} (agent={agent_role}, task={task_type})"
                )
            ...
            logger.info(
                f"模型选择完成: {model_selection.model_spec.name} (置信度: {model_selection.confidence_score:.3f})"
            )
            ...
            try:
                available_models = self._get_all_available_models()
                candidates = (
                    self._filter_candidates_by_policy(
                        agent_role, task_type, available_models, context or {}
                    )
                    or available_models
                )
                if candidates:
    ...
    def _get_locked_model(self, agent_role: str, context: dict[str, Any]) -> str | None:
        """获取锁定模型，优先级：context > runtime_overrides > agent_bindings.locked_model"""
        # 来自每次请求上下文
        overrides = context.get("model_overrides") or {}
        if (
            isinstance(overrides, dict)
            and agent_role in overrides
            and overrides[agent_role]
        ):
    ...
    def _filter_candidates_by_policy(
        ...
        allowed_from_agent = set(
            self.agent_bindings.get(agent_role, {}).get("allow_models", [])
        )
        denied_from_agent = set(
            self.agent_bindings.get(agent_role, {}).get("deny_models", [])
        )

        task_cfg = (
            self.task_bindings.get(task_type, {})
            if isinstance(self.task_bindings, dict)
            else {}
        )
        ...
        if isinstance(context.get("allowed_models_by_role"), dict):
            rt_allowed_by_role = set(
                context["allowed_models_by_role"].get(agent_role, []) or []
            )

        # 全局运行时允许列表（启用时）
        rt_cfg = self.runtime_overrides or {}
        global_allowed = None
        if rt_cfg.get("enable_allowed_models_by_role") and isinstance(
            rt_cfg.get("allowed_models_by_role"), dict
        ):
            global_allowed = set(
                rt_cfg["allowed_models_by_role"].get(agent_role, []) or []
            )
        ...
        try:
            ...
            if model_override:
                ...
                if chosen_name not in available_models:
                    ...
                    if isinstance(chosen_name, str) and chosen_name.startswith(
                        "gemini-api/"
                    ):
            ...
            if not client:
                raise ValueError(
                    f"找不到模型 {model_selection.model_spec.name} 对应的客户端"
                )
            ...
            try:
                self._record_token_usage(
                    result, model_selection, context, session_id, task_type
                )
            except Exception as track_err:
                logger.debug(f"记录Token使用失败: {track_err}")

            logger.info(
                f"任务执行完成: {result.model_used.name if result.model_used else 'unknown'}, 成功: {result.success}"
            )
            ...
            fallback_result = self._attempt_task_fallback(
                e, agent_role, task_prompt, context
            )
            if fallback_result:
                ...
                try:
                    # 尝试为降级结果也记录使用
                    self._record_token_usage(
                        fallback_result, None, context, session_id, task_type
                    )
                except Exception:
                    pass
    ...
    def _record_token_usage(
        ...
        if selection and selection.model_spec:
            ...
            provider_name = (
                result.model_used.provider.value
                if hasattr(result.model_used, "provider")
                else None
            )
        ...
        if provider_name is None:
            provider_name = (
                ...
                else "deepseek" if "deepseek" in model_name.lower() else "siliconflow"
            )
        ...
        # 当缺少精确用量时进行保守估算
        if (input_tokens == 0 and output_tokens == 0) and isinstance(
            result.result, str
        ):
    ...
    def execute_collaborative_analysis(
        ...
        try:
            logger.info(
                f"开始协作分析: {collaboration_mode} 模式，参与者: {participating_agents}"
            )
...
请提供你的专业分析和建议。"""
        ...
        # 生成最终结果汇总
        final_result = self._synthesize_sequential_results(
            individual_results, task_description
        )
    ...
    def _execute_parallel_collaboration(
        ...
        for agent_role in participating_agents:
            prompt = (
                f"请作为{agent_role}从你的专业角度分析以下任务：\n{task_description}"
            )
        ...
        # 并行执行的总时间是最长的单个任务时间
        total_time = max(
            (result.execution_time for result in individual_results), default=0
        )

        # 生成最终结果汇总
        final_result = self._synthesize_parallel_results(
            individual_results, task_description
        )
...
请明确表达你的立场和理由。"""
            ...
            debate_history.append(
                {"round": 1, "agent": agent_role, "position": result.result}
            )
        ...
        for round_num in range(2, max_rounds + 1):
            for i, agent_role in enumerate(participating_agents):
                ...
                other_positions = [
                    entry["position"]
                    for entry in debate_history[-len(participating_agents) :]
...
"""
                ...
                debate_history.append(
                    {"round": round_num, "agent": agent_role, "position": result.result}
                )
    ...
    def _get_client_for_model(self, model_name: str) -> BaseMultiModelAdapter | None:
        """根据模型名称获取对应的客户端"""
        for client in self.clients.values():
    ...
    def _get_default_model(
        ...
        if task_spec.requires_speed:
            ...
            if speed_models:
                ...
                logger.info(f"选择速度优先默认模型: {selected}")
    ...
    def _estimate_tokens(self, text: str) -> int:
        ...
        english_words = len(
            [word for word in text.split() if any(c.isalpha() for c in word)]
        )
    ...
    def _requires_reasoning(self, task_type: str, description: str) -> bool:
        ...
        return task_type in reasoning_types or any(
            keyword in description.lower() for keyword in reasoning_keywords
        )
    ...
    def _requires_speed(self, context: dict[str, Any]) -> bool:
        """判断任务是否需要快速响应"""
        return (
            context.get("priority") == "high"
            or context.get("time_limit") is not None
            or context.get("real_time", False)
        )
    ...
    def _synthesize_sequential_results(
        self, results: list[TaskResult], original_task: str
    ) -> str:
        """合成序列协作的结果"""
        if not results:
            return "无法生成分析结果"
...
"""
    ...
    def _synthesize_debate_results(
        self, debate_history: list[dict], original_task: str
    ) -> str:
        """合成辩论协作的结果"""
...
"""
    ...
    def _update_session_metrics(
        self, session_id: str, result: TaskResult, selection: ModelSelection
    ) -> None:
        """更新会话指标"""
        ...
        session.avg_confidence = (
            session.avg_confidence * (session.total_tasks - 1)
    ...
    def _execute_task_with_fallback(
        ...
        logger.warning(
            f"主模型 {model_selection.model_spec.name} 执行失败，尝试备用模型"
        )

        # 2. 获取智能路由的备用模型
        alternative_models = self._get_alternative_models(
            agent_role, task_type, task_spec, context
        )
        ...
        for attempt, model_name in enumerate(alternative_models[:max_attempts]):
            ...
            try:
                result = self._try_execute_with_model(
                    model_name, task_prompt, task_spec, context
                )
    ...
    def _update_routing_performance(
        self, model_spec: ModelSpec, task_type: str, result: TaskResult
    ):
        """更新路由引擎性能数据"""
            ...
            for attempt in range(max_attempts):
                for candidate in fallback_candidates:
                    ...
                    try:
                        ...
                        # 3. 简化任务提示词
                        simplified_prompt = self._simplify_task_prompt(
                            task_prompt, agent_role
                        )
                        ...
                        if result.success:
                            ...
                            logger.info(
                                f"任务降级成功: {selected_model.name} (尝试 {attempt+1})"
                            )
                            return result

                    except Exception as model_error:
                        logger.warning(
                            f"降级模型 {candidate} 执行失败 (尝试 {attempt+1}): {model_error}"
                        )
                        continue
    ...
    def _attempt_collaboration_fallback(
        self,
        original_error: Exception,
        task_description: str,
        participating_agents: list[str],
        ...
        try:
            ...
            simplified_context.update(
                {"fallback_mode": True, "max_attempts": 1, "timeout": 30}  # 30秒超时
            )
    ...
    def _simplify_task_prompt(self, original_prompt: str, agent_role: str) -> str:
        ...
        # 限制内容长度
        content = (
            original_prompt[:800] if len(original_prompt) > 800 else original_prompt
        )
    ...
    def get_system_health_status(self) -> dict[str, Any]:
        ...
        try:
            health_status = {
                ...
                "total_models": sum(
                    len(client.get_supported_models())
                    for client in self.clients.values()
                ),
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            status = {
                ...
                "routing_engine_status": (
                    "active" if self.routing_engine else "inactive"
                ),
        ...
        status = {
            "active_clients": list(self.clients.keys()),
            "total_models": sum(
                len(client.get_supported_models()) for client in self.clients.values()
            ),
            "active_sessions": len(self.active_sessions),

...
class BaseMultiModelAdapter(ABC):
    ...
    def get_model_fitness_score(self, model_name: str, task_spec: TaskSpec) -> float:
        """
        计算模型对特定任务的适配度评分
        ...
        # 任务类型匹配度
        type_bonus = 0.0
        if task_spec.requires_reasoning and model.model_type in [
            "reasoning",
            "premium",
        ]:
            type_bonus += 0.2
    ...
    def update_performance_metrics(
        self, model_name: str, task_type: str, execution_time: int, success: bool
    ) -> None:
        """更新模型性能指标缓存"""

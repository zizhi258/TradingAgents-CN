...
class PostProcessingOrchestrator:
    ...
    def register_completion_callback(
        self, callback: Callable[[PostProcessingResult], None]
    ) -> None:
        """注册后处理完成回调函数"""
        self.completion_callbacks.append(callback)

    def should_trigger_post_processing(
        self, analysis_results: dict[str, AgentAnalysisResult]
    ) -> bool:
        """
        判断是否应该触发后处理
    ...
    async def execute_post_processing(
        ...
        try:
            ...
            result = PostProcessingResult(
                success=True,
                context=context,
                charts_generated=context.visualization_results.get(
                    "charts_generated", []
                ),
                total_execution_time=int(
                    (context.completion_time - context.start_time).total_seconds()
                    * 1000
                ),
            )

            self.logger.info(
                f"后处理完成: {len(result.charts_generated)} 个图表, 耗时: {result.total_execution_time}ms"
            )
    ...
    async def _prepare_analysis_data(self, context: PostProcessingContext) -> None:
        ...
        try:
            ...
            visualization_data = self._extract_visualization_data(
                combined_results, context.market_data
            )
    ...
    def _extract_visualization_data(
        self, analysis_results: dict[str, Any], market_data: dict[str, Any]
    ) -> dict[str, Any]:
        """从分析结果中提取可视化数据"""
        viz_data = {}
        ...
        try:
            ...
            if "technical_analyst" in analysis_results:
                tech_data = analysis_results["technical_analyst"].get(
                    "supporting_data", {}
                )
            ...
            if "fundamental_expert" in analysis_results:
                fund_data = analysis_results["fundamental_expert"].get(
                    "supporting_data", {}
                )
    ...
    def get_task_status(self, session_id: str) -> PostProcessingStage | None:
        """获取指定任务的状态"""
    ...
    def get_performance_stats(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {"error": str(e), "orchestrator_enabled": False}


# 全局编排器实例管理
_orchestrator_instance: PostProcessingOrchestrator | None = None


def initialize_post_processing_orchestrator(
    multi_model_manager, config: dict[str, Any] = None
    ...
    if _orchestrator_instance is None:
        _orchestrator_instance = PostProcessingOrchestrator(
            multi_model_manager=multi_model_manager, config=config
        )
...
def get_post_processing_orchestrator() -> PostProcessingOrchestrator | None:
    """获取全局后处理编排器实例"""
    return _orchestrator_instance


# 便捷函数
async def execute_post_processing_if_needed(
    session_id: str,
    symbol: str,
    analysis_results: dict[str, AgentAnalysisResult],
    market_data: dict[str, Any] = None,

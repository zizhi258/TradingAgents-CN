...
class OrchestratorConfig:
    ...
    analysis_focus: dict[str, bool] | None = None
    ai_model_config: dict[str, Any] | None = None
    advanced_options: dict[str, Any] | None = None


class MarketScanOrchestrator:
    def __init__(self, progress_cb: Callable[[str, int, int], None] | None = None):
    ...
    def run(self, cfg: OrchestratorConfig) -> dict[str, Any]:
        ...
        def _eval_symbol(sym: str) -> dict[str, Any]:
            try:
                if ta_graph is not None:
                    ...
                    use_multi = bool(llm_cfg.get("use_ensemble")) or (
                        os.getenv("MULTI_MODEL_ENABLED", "false").lower() == "true"
                    )
                    collab_mode = (
                        llm_cfg.get("collaboration_mode")
                        or os.getenv("DEFAULT_COLLABORATION_MODE")
                        or "sequential"
                    )
        ...
        with futures.ThreadPoolExecutor(max_workers=max_workers) as ex:
            for res in ex.map(_eval_symbol, symbols):
                completed += 1
                self._emit(
                    f"评估进度 {completed}/{total}",
                    12 + int(70 * completed / max(1, total)),
                    100,
                )
                if "__error__" in res:
        ...
        summary = self._build_summary(
            cfg, rankings, sectors, breadth, use_llm, ta_graph
        )
        ...
        stats = {
            ...
            "recommended_stocks": sum(
                1 for r in rankings if r.get("recommendation") == "买入"
            ),
            "actual_cost": round(
                min(cfg.budget_limit or 0.0, (len(rankings) * 0.002)), 4
            ),
    ...
    def _analyze_llm_config(
        self, ai_cfg: dict[str, Any] | None
    ...
    def _map_decision_to_row(self, symbol: str, decision: Any) -> dict[str, Any]:
        ...
        try:
            ...
            if isinstance(decision, dict):
                rec = decision.get("final_trade_decision", "") or decision.get(
                    "recommendation", "关注"
                )
    ...
    def _map_multi_model_result_to_row(
        ...
        try:
            ...
            if isinstance(rec, str):
                ...
                base["recommendation"] = mapping.get(
                    rec_l, rec if rec in ["买入", "持有", "关注", "卖出"] else "关注"
                )
            else:
        ...
        for sec, items in sectors.items():
            ...
            out[sec] = {
                ...
                "recommended_count": sum(
                    1 for i in items if i.get("recommendation") == "买入"
                ),
    ...
    def _build_breadth(self, rankings: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        senti = min(
            ...
            max(
                30.0,
                sum(1 for r in rankings if r.get("recommendation") == "买入")
                * 100.0
                / n,
            ),
        )
    ...
    def _build_summary(
        ...
        return {
            ...
            "investment_recommendations": {
                "buy": [
                    {"symbol": r["symbol"], "reason": "综合评分较高"} for r in top_buys
                ],
                "watch": [
                    {"symbol": r["symbol"], "reason": "具备一定潜力"}
                    for r in rankings[:5]
                    if r not in top_buys
                ],
            },
            "risk_factors": [
                "个股波动与流动性风险需关注",

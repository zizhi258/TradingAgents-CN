...
class GeminiAnalysisValidator:
    ...
    def validate_model_parameters(
        ...
        """
        logger.info(
            f"🧪 Starting validation for {model_name} with parameters: {parameters}"
        )
        ...
        if baseline_parameters:
            logger.info(
                f"🔄 Running baseline tests with parameters: {baseline_parameters}"
            )
            baseline_results = self._run_test_suite(
                model_name, baseline_parameters, is_baseline=True
            )
    ...
    def _run_test_suite(
        self, model_name: str, parameters: dict[str, Any], is_baseline: bool = False
        ...
        logger.info(
            f"🎯 Running {test_prefix} test suite with {len(self.test_cases)} test cases"
        )
    ...
    def _run_single_test(
        self, model_name: str, parameters: dict[str, Any], test_case: ValidationTestCase
    ) -> ValidationResult:
        ...
        llm = ChatGoogleGenerativeAI(
            model=model_name, google_api_key=google_api_key, **parameters
        )
        ...
        if hasattr(response, "usage_metadata"):
            actual_input_tokens = getattr(
                response.usage_metadata, "input_tokens", input_tokens
            )
            actual_output_tokens = getattr(
                response.usage_metadata, "output_tokens", output_tokens
            )
        ...
        contains_keywords = [
            kw for kw in test_case.expected_keywords if kw.lower() in response_lower
        ]
        missing_keywords = [
            kw for kw in test_case.expected_keywords if kw.lower() not in response_lower
        ]

        # Determine success
        success = (
            len(response_content) >= test_case.min_response_length
            and len(contains_keywords)
            >= len(test_case.expected_keywords) * 0.7  # 70% keyword coverage
            and estimated_cost <= test_case.max_acceptable_cost
        )
    ...
    def _calculate_cost(
        ...
        cost = config_manager.calculate_cost(
            "google", model_name, input_tokens, output_tokens
        )
        if cost == 0.0:
    ...
    def _generate_validation_report(
        ...
        baseline_results: list[ValidationResult] | None,
    ) -> dict[str, Any]:
        ...
        report = {
            ...
            "baseline_results": (
                {
                    ...
                    "detailed_results": (
                        [self._result_to_dict(r) for r in baseline_results]
                        if baseline_results
                        else []
                    ),
                }
            ...
            "overall_assessment": self._assess_overall_performance(
                current_metrics, improvement_analysis
            ),
        }
    ...
    def _calculate_metrics(self, results: list[ValidationResult]) -> dict[str, Any]:
        ...
        for result in results:
            test_case = next(
                tc for tc in self.test_cases if tc.name == result.test_case_name
            )
            coverage = (
                len(result.contains_keywords) / len(test_case.expected_keywords)
        ...
        avg_keyword_coverage = (
            sum(keyword_coverage_rates) / len(keyword_coverage_rates)
            if keyword_coverage_rates
            else 0
        )
        ...
        return {
            ...
            "cost_efficiency": (
                avg_response_length / total_cost if total_cost > 0 else 0
            ),  # chars per dollar
        }
    ...
    def _analyze_improvements(
        ...
        if "avg_cost_per_test" in current and "avg_cost_per_test" in baseline:
            if baseline["avg_cost_per_test"] > 0:
                cost_increase_pct = (
                    (current["avg_cost_per_test"] - baseline["avg_cost_per_test"])
                    / baseline["avg_cost_per_test"]
                ) * 100

        improvements["cost_increase_pct"] = cost_increase_pct
        improvements["cost_acceptable"] = (
            cost_increase_pct <= 100
        )  # No more than 100% increase
    ...
    def _generate_recommendations(
        ...
        improvement_analysis: dict[str, Any] | None,
    ) -> list[str]:
        ...
        if metrics.get("success_rate", 0) < 0.8:
            recommendations.append(
                "❌ Success rate below 80% - consider adjusting parameters"
            )
        elif metrics.get("success_rate", 0) >= 0.9:
            recommendations.append(
                "✅ Excellent success rate - parameters are well-tuned"
            )

        # Cost recommendations
        if metrics.get("avg_cost_per_test", 0) > 1.0:
            recommendations.append(
                "💰 Average cost per test exceeds $1.00 - consider cost optimization"
            )

        # Response quality recommendations
        if metrics.get("avg_keyword_coverage", 0) < 0.7:
            recommendations.append(
                "📝 Low keyword coverage - responses may be incomplete"
            )
        ...
        if improvement_analysis:
            if improvement_analysis.get("cost_increase_pct", 0) > 100:
                recommendations.append(
                    "⚠️ Cost increased by >100% - validate if benefits justify the expense"
                )

            if not improvement_analysis.get("success_rate_improved", False):
                recommendations.append(
                    "⚠️ Success rate did not improve - current parameters may not be effective"
                )

        # Parameter-specific recommendations
        if not recommendations:
            recommendations.append(
                "✅ All metrics within acceptable ranges - parameters appear optimal"
            )

        return recommendations

    def _assess_overall_performance(
        self, metrics: dict[str, Any], improvement_analysis: dict[str, Any] | None
    ) -> str:
        ...
        if improvement_analysis:
            ...
            if (
                success_rate >= 0.8
                and keyword_coverage >= 0.7
                and success_improved
                and cost_increase <= 100
            ):
            ...
            if success_rate >= 0.8 and keyword_coverage >= 0.7 and avg_cost <= 1.0:
                return (
                    "EXCELLENT - Parameters provide good performance at reasonable cost"
                )
...
if __name__ == "__main__":
    ...
    if results["baseline_results"]:
        ...
        print(
            f"  Success Rate Improved: {improvement.get('success_rate_improved', False)}"
        )

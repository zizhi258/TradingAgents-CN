...
def create_bull_researcher(llm, memory):
    def bull_node(state) -> dict:
        ...
        logger.debug(
            f"🐂 [DEBUG] - 股票代码: {company_name}, 类型: {market_info['market_name']}, 货币: {currency}"
        )
        logger.debug(
...
"""
        ...
        try:
            ...
            logger.debug(f"🐂 [DEBUG] 看涨发言包含引用标记: {has_citation}")
            try:
                from tradingagents.monitoring.debate_metrics import record_citation

                record_citation("bull", bool(has_citation))
            except Exception:
                pass
        except Exception:
            has_citation = False

        # 提取简单的工具与引用标记，写入可选 tool_calls / cross_question
        tool_calls = []
        citations = []
        try:
            import re

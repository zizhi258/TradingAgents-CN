...
通过批判性地分析双方来积极参与，解决激进和保守论点中的弱点，倡导更平衡的方法。挑战他们的每个观点，说明为什么适度风险策略可能提供两全其美的效果，既提供增长潜力又防范极端波动。专注于辩论而不是简单地呈现数据，旨在表明平衡的观点可以带来最可靠的结果。请用中文以对话方式输出，就像您在说话一样，不使用任何特殊格式。{react_instructions}{crossq_note}"""
        ...
        try:
            ...
            logger.debug(f"⚖️ [DEBUG] 中性发言包含引用标记: {has_citation}")
            try:
                from tradingagents.monitoring.debate_metrics import record_citation

                record_citation("neutral", bool(has_citation))
            except Exception:
                pass
        except Exception:
            has_citation = False

...
def create_market_analyst_react(llm, toolkit):
    ...
    def market_analyst_react_node(state):
        logger.debug("📈 [DEBUG] ===== ReAct市场分析师节点开始 =====")
        ...
        if toolkit.config["online_tools"]:
            # 在线模式，使用ReAct Agent
            if is_china:
                logger.info("📈 [市场分析师] 使用ReAct Agent分析中国股票")
                ...
                class ChinaStockDataTool(BaseTool):
                    ...
                    description: str = (
                        f"获取中国A股股票{ticker}的市场数据和技术指标（优化缓存版本）。直接调用，无需参数。"
                    )
...
## 投资建议"""
                ...
                logger.info("📈 [市场分析师] 使用ReAct Agent分析美股/港股")
                ...
                class USStockDataTool(BaseTool):
                    ...
                    description: str = (
                        f"获取美股/港股{ticker}的市场数据和技术指标（优化缓存版本）。直接调用，无需参数。"
                    )
                ...
                class FinnhubNewsTool(BaseTool):
                    ...
                    description: str = (
                        f"获取美股{ticker}的最新新闻和市场情绪（通过FINNHUB API）。直接调用，无需参数。"
                    )
...
## 投资建议"""
            ...
            try:
                ...
                logger.debug("📈 [DEBUG] 执行ReAct Agent查询...")
        ...
        logger.debug("📈 [DEBUG] ===== ReAct市场分析师节点结束 =====")

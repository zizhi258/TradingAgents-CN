...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        if hasattr(result, "content") and result.content:
            ...
            if "002021" in content:
                logger.warning(
                    "🔍 [股票代码追踪] 警告：LLM返回内容中包含错误股票代码 002021"
                )
                logger.warning(
                    f"🔍 [股票代码追踪] LLM返回内容前500字符: {content[:500]}..."
                )
            if "002027" in content:
                logger.info("🔍 [股票代码追踪] LLM返回内容中包含正确股票代码 002027")
        ...
        if hasattr(result, "tool_calls") and len(result.tool_calls) > 0:
            ...
            logger.debug("📊 [DEBUG] 检测到模型未调用工具，启用强制工具调用模式")
            ...
            try:
                ...
                if unified_tool:
                    ...
                    if "002021" in combined_data:
                        logger.warning(
                            "🔍 [股票代码追踪] 警告：统一工具返回数据中包含错误股票代码 002021"
                        )
                    if "002027" in combined_data:
                        logger.info(
                            "🔍 [股票代码追踪] 统一工具返回数据中包含正确股票代码 002027"
                        )
                else:
            ...
            # 生成基于真实数据的分析报告
            logger.info(
                f"🔍 [股票代码追踪] 生成分析提示词，使用ticker: '{ticker}', company_name: '{company_name}'"
            )
...
- 分析要详细且专业"""
            ...
            try:
                ...
                custom_analysis_tpl = get_prompt(
                    "fundamental_expert", "analysis_prompt_template"
                )
            ...
            try:
                ...
                # 检查最终报告中的股票代码并进行修正
                logger.info("🔍 [股票代码追踪] 最终报告生成完成，检查股票代码...")
                    ...
                    if correct_code in error_mappings:
                        for wrong_code in error_mappings[correct_code]:
                            if wrong_code in content:
                                logger.warning(
                                    f"🔍 [股票代码验证] 发现错误代码 {wrong_code}，修正为 {correct_code}"
                                )
                ...
                if report != original_report:
                    logger.info("🔍 [股票代码验证] 已修正报告中的错误股票代码")

                if "002021" in report:
                    logger.warning(
                        "🔍 [股票代码追踪] 警告：最终报告中仍包含错误股票代码 002021"
                    )
                    logger.warning(
                        f"🔍 [股票代码追踪] 最终报告前500字符: {report[:500]}..."
                    )
                if "002027" in report:
                    logger.info("🔍 [股票代码追踪] 最终报告中包含正确股票代码 002027")

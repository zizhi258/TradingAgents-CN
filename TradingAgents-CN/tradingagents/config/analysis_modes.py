...
from .provider_models import CollaborationMode, RoutingStrategy, model_provider_manager
...
class MethodConfig:
    ...
    # 单模型配置
    single_model_provider: str | None = None
...
class AnalysisModeManager:
    ...
    def create_multi_model_config(
        ...
        role_model_policies: dict[str, Any] | None = None,
        max_budget: float = 2.0,
    ) -> MethodConfig:
        """创建多模型配置"""
        ...
        if role_model_policies is None:
            role_model_policies = self._get_default_role_model_policies(
                routing_strategy
            )
    ...
    def _get_default_role_model_policies(
        ...
        policies = {"allowed_models_by_role": {}, "model_overrides": {}}
        ...
        for role_key in self.model_manager.role_definitions.keys():
            ...
            if role_config:
                # 获取允许的模型列表
                policies["allowed_models_by_role"][
                    role_key
                ] = role_config.allowed_models
                ...
                if role_config.locked_model:
                    ...
                    # 根据策略选择最佳模型
                    best_model = self.model_manager.get_best_model_for_role(
                        role_key, strategy
                    )
                    if best_model:
    ...
    def merge_config_with_overrides(
        self, base_config: MethodConfig, user_overrides: dict[str, Any]
    ) -> MethodConfig:
        """合并用户覆盖配置"""
        ...
        # 多模型模式验证
            ...
            if not config.role_model_policies:
                ...
                is_valid, validation_errors = (
                    self.model_manager.validate_role_model_assignment(model_overrides)
                )
                if not is_valid:
    ...
    def get_preset_configs(self) -> dict[str, MethodConfig]:
        ...
        return {
            "single_fast": self.create_single_model_config(
                provider="deepseek", model="deepseek-chat", max_budget=0.5
            ),
            "single_quality": self.create_single_model_config(
                provider="google", model="gemini-2.5-pro", max_budget=1.0
            ),

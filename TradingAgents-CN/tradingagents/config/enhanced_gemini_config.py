...
class EnhancedGeminiConfigManager:
    ...
    def _load_cost_monitoring_config(self) -> CostMonitoringConfig:
        ...
        if self.cost_monitoring_file.exists():
            try:
                ...
                with open(self.cost_monitoring_file) as f:
                    data = json.load(f)
                    default_config = CostMonitoringConfig(**data)
            except Exception as e:
                logger.error(
                    f"Failed to load cost monitoring config, using defaults: {e}"
                )
    ...
    def _load_usage_history(self):
        ...
        try:
            ...
            self.usage_history = recent_stats.get("provider_stats", {}).get(
                "google", {}
            )
        except Exception as e:
    ...
    def get_optimal_parameters(
        ...
        """
        logger.info(
            f"🔧 Getting optimal parameters for {model_name}, task: {task_type}, complexity: {complexity_level}"
        )
        ...
        if not cost_check["allowed"]:
            logger.warning(
                f"⚠️ Cost limits exceeded for {model_name}: {cost_check['reason']}"
            )
        ...
        max_tokens = self._apply_cost_scaling(
            max_tokens, model_name, cost_check["remaining_budget_pct"]
        )
        ...
        parameters["_cost_monitoring"] = {
            ...
            "original_max_tokens": (
                config.deep_thinking_max_tokens
                if complexity_level == "high"
                else config.base_max_tokens
            ),
        }
    ...
    def _check_cost_limits(
        self, model_name: str, current_usage: dict[str, Any] | None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            daily_cost = (
                daily_stats.get("total_cost", 0) * 0.14
            )  # CNY to USD rough conversion
            ...
            daily_limit_exceeded = (
                daily_cost
                >= self.cost_monitoring.daily_budget_usd
                * (self.cost_monitoring.emergency_stop_threshold_pct / 100)
            )
            weekly_limit_exceeded = (
                weekly_cost
                >= self.cost_monitoring.weekly_budget_usd
                * (self.cost_monitoring.emergency_stop_threshold_pct / 100)
            )
            monthly_limit_exceeded = (
                monthly_cost
                >= self.cost_monitoring.monthly_budget_usd
                * (self.cost_monitoring.emergency_stop_threshold_pct / 100)
            )
            ...
            daily_remaining = max(
                0,
                (self.cost_monitoring.daily_budget_usd - daily_cost)
                / self.cost_monitoring.daily_budget_usd
                * 100,
            )
            weekly_remaining = max(
                0,
                (self.cost_monitoring.weekly_budget_usd - weekly_cost)
                / self.cost_monitoring.weekly_budget_usd
                * 100,
            )
            monthly_remaining = max(
                0,
                (self.cost_monitoring.monthly_budget_usd - monthly_cost)
                / self.cost_monitoring.monthly_budget_usd
                * 100,
            )

            remaining_budget_pct = min(
            ...
            if remaining_budget_pct <= (100 - self.cost_monitoring.alert_threshold_pct):
                logger.warning(
                    f"⚠️ Cost alert: {remaining_budget_pct:.1f}% budget remaining"
                )
    ...
    def _get_fallback_parameters(
        self, model_name: str, cost_check: dict[str, Any]
    ) -> dict[str, Any]:
        """Get reduced parameters when cost limits are exceeded"""
        logger.warning(
            f"🔥 Using fallback parameters for {model_name} due to cost limits"
        )
    ...
    def _apply_adaptive_scaling(
        ...
        if config and config.daily_token_limit:
            daily_usage = self.usage_history.get(
                "input_tokens", 0
            ) + self.usage_history.get("output_tokens", 0)
            remaining_daily_tokens = config.daily_token_limit - daily_usage
            scaled_tokens = min(
                scaled_tokens, remaining_daily_tokens // 4
            )  # Reserve 75% for other requests

        logger.debug(
            f"Adaptive scaling: {base_max_tokens} -> {scaled_tokens} (factor: {scale_factor:.2f})"
        )
        return max(1000, scaled_tokens)  # Minimum 1000 tokens

    def _apply_cost_scaling(
        self, max_tokens: int, model_name: str, remaining_budget_pct: float
    ) -> int:
        ...
        logger.debug(
            f"Cost scaling: {max_tokens} -> {scaled_tokens} (budget: {remaining_budget_pct:.1f}%)"
        )
    ...
    def get_cost_report(self, days: int = 7) -> dict[str, Any]:
        ...
        report = {
            ...
            "total_tokens": google_stats.get("input_tokens", 0)
            + google_stats.get("output_tokens", 0),
            "avg_cost_per_request": cost_usd / max(1, google_stats.get("requests", 1)),
            "budget_utilization": {
                "daily": min(
                    100, (cost_usd / days) / self.cost_monitoring.daily_budget_usd * 100
                ),
                "weekly": (
                    min(100, cost_usd / self.cost_monitoring.weekly_budget_usd * 100)
                    if days >= 7
                    else 0
                ),
                "monthly": (
                    min(100, cost_usd / self.cost_monitoring.monthly_budget_usd * 100)
                    if days >= 30
                    else 0
                ),
            },
...
def get_enhanced_gemini_parameters(
    model_name: str, task_type: str = "stock_analysis", complexity_level: str = "medium"
) -> dict[str, Any]:
    """
    ...
    return enhanced_gemini_config.get_optimal_parameters(
        model_name=model_name, task_type=task_type, complexity_level=complexity_level
    )

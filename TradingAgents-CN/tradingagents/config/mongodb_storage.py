...
class MongoDBStorage:
    ...
    def __init__(self, connection_string: str = None, database_name: str = "tradingagents"):
        ...
        # 修复硬编码问题 - 检查多个可能的环境变量
        self.connection_string = connection_string or os.getenv("MONGODB_CONNECTION_STRING") or os.getenv("TRADINGAGENTS_MONGODB_URL")
        if not self.connection_string:
            raise ValueError(
                "MongoDB连接字符串未配置。请通过以下方式之一进行配置：\n"
                "1. 设置环境变量 MONGODB_CONNECTION_STRING 或 TRADINGAGENTS_MONGODB_URL\n"
    ...
    # Generic database methods for subscription manager and other components
    def find_one(self, collection_name: str, query: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(f"MongoDB find_one失败: {e}")
            return None
    
    def find(self, collection_name: str, query: Dict[str, Any] = None, sort: List = None, limit: int = None) -> List[Dict[str, Any]]:
        """查找多个文档"""
        if not self._connected:
            return []
        ...
        try:
            ...
            logger.error(f"MongoDB find失败: {e}")
            return []
    
    def insert(self, collection_name: str, document: Dict[str, Any]) -> Optional[str]:
        """插入单个文档"""
        if not self._connected:
            return None
        ...
        try:
            ...
            logger.error(f"MongoDB insert失败: {e}")
            return None
    
    def update_one(self, collection_name: str, query: Dict[str, Any], update: Dict[str, Any]) -> Any:
        """更新单个文档"""
        if not self._connected:
            return None
        ...
        try:
            ...
            logger.error(f"MongoDB update_one失败: {e}")
            return None
    
    def update_many(self, collection_name: str, query: Dict[str, Any], update: Dict[str, Any]) -> Any:
        """更新多个文档"""
        if not self._connected:
            return None
        ...
        try:
            ...
            logger.error(f"MongoDB update_many失败: {e}")
            return None
    ...
    def count(self, collection_name: str, query: Dict[str, Any] = None) -> int:
        ...
        try:
            ...
            logger.error(f"MongoDB count失败: {e}")
            return 0
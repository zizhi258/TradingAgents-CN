...
class MongoDBStorage:
    ...
    def __init__(
        ...
        self.connection_string = (
            ...
            or os.getenv("TRADINGAGENTS_MONGODB_URL")
        )
    ...
    def _connect(self):
        ...
        try:
            self.client = MongoClient(
                self.connection_string, serverSelectionTimeoutMS=5000  # 5秒超时
            )
            # 测试连接
            ...
            logger.info(
                f"✅ MongoDB连接成功: {self.database_name}.{self.collection_name}"
            )
    ...
    def _create_indexes(self):
        ...
        try:
            ...
            self.collection.create_index(
                [("timestamp", -1), ("provider", 1), ("model_name", 1)]  # 按时间倒序
            )
    ...
    def save_usage_record(self, record: UsageRecord) -> bool:
        ...
        try:
            ...
            if result.inserted_id:
                ...
                logger.error("MongoDB插入失败：未返回插入ID")
                return False
    ...
    def load_usage_records(
        self, limit: int = 10000, days: int = None
    ) -> list[UsageRecord]:
        """从MongoDB加载使用记录"""
        if not self._connected:
            return []
    ...
    def insert(self, collection_name: str, document: dict[str, Any]) -> str | None:
        """插入单个文档"""
        if not self._connected:
            return None

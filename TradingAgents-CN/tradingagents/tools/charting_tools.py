...
class ChartGeneratorTools:
    ...
    def generate_candlestick_chart(
        ...
        try:
            ...
            fig = make_subplots(
                ...
                subplot_titles=(
                    [f"{symbol} K线图", "成交量"] if rows == 2 else [f"{symbol} K线图"]
                ),
            )
            ...
            if "volume" in indicators and rows == 2:
                colors = [
                    ...
                    for close, open in zip(data["close"], data["open"], strict=False)
                ]
    ...
    def generate_technical_analysis_chart(
        ...
        try:
            if analysis_type == "macd":
                ...
                return self._generate_comprehensive_technical_chart(
                    data, symbol, config
                )
            else:
    ...
    def generate_financial_dashboard(
        self, financial_data: dict[str, Any], symbol: str, config: dict[str, Any] = None
    ) -> dict[str, Any]:
        """生成财务仪表板"""
        ...
        try:
            ...
            for edge in G.edges():
                ...
                corr = next(
                    ...
                    or (item["source"] == edge[1] and item["target"] == edge[0])
                )
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    marker=dict(
                        size=30, color="lightblue", line=dict(width=2, color="black")
                    ),
                    name="资产节点",
                )
            )
    ...
    def _save_chart(
        self, fig: go.Figure, chart_name: str, config: dict[str, Any]
    ) -> dict[str, Any]:
        """保存图表到文件"""
        try:
            # 生成唯一ID
            ...
            if export_format == "html":
                ...
                fig.write_image(
                    str(file_path), width=config["width"], height=config["height"]
                )
        ...
        try:
            ...
            for chart_file in chart_files:
                if chart_file.is_file():
                    ...
                    stats["chart_types"][chart_type] = (
                        stats["chart_types"].get(chart_type, 0) + 1
                    )
...
# 导出主要类
__all__ = ["ChartGeneratorTools", "ChartManagementTools"]

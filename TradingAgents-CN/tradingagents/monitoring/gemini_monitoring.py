...
class ParameterEffectivenessTracker:
    ...
    def __init__(self):
        self.effectiveness_data = defaultdict(
            lambda: {
                "usage_count": 0,
                "total_cost": 0.0,
    ...
    def record_usage(
        ...
        data["avg_response_length"] = (
            prev_avg * (data["usage_count"] - 1) + response_length
        ) / data["usage_count"]
    ...
    def get_effectiveness_report(self, parameter_set_id: str) -> dict[str, Any]:
        ...
        success_rate = data["success_indicators"] / max(
            1, data["success_indicators"] + data["failure_indicators"]
        )
        ...
        return {
            ...
            "cost_per_quality_point": avg_cost_per_use
            / max(0.1, avg_quality_score or 0.1),
    ...
    def compare_parameter_sets(self) -> dict[str, Any]:
        ...
        for param_set_id, report in reports.items():
            if "error" not in report and report["avg_quality_score"]:
                efficiency = report["avg_quality_score"] / max(
                    0.001, report["avg_cost_per_use"]
                )
...
class AnalysisQualityMonitor:
    ...
    def assess_response_quality(
        self, response: str, expected_elements: list[str] = None
    ) -> dict[str, Any]:
        ...
        quality_score = {
            ...
            "numeric_mentions": len(
                [w for w in response.split() if any(c.isdigit() for c in w)]
            ),
        ...
        if expected_elements:
            found_elements = sum(
                1 for elem in expected_elements if elem.lower() in response.lower()
            )
            quality_score["completeness_score"] = found_elements / len(
                expected_elements
            )
    ...
    def _check_structured_content(self, response: str) -> dict[str, bool]:
        ...
        return {
            "has_headers": any(
                line.startswith("#") or line.endswith(":") for line in lines
            ),
            "has_lists": any(line.startswith(("-", "*", "•")) for line in lines),
            "has_numbered_lists": any(
                line[0].isdigit() and "." in line[:3] for line in lines
            ),
    ...
    def _check_threshold_violations(
        ...
        for score in scores:
            if (
                score["completeness_score"] > 0
                and score["completeness_score"]
...
class CostEffectivenessMonitor:
    ...
    def _check_budget_alerts(self):
        ...
        if daily_cost > self.budget_alerts["daily_budget"] * 0.8:  # 80% threshold
            logger.warning(
                f"⚠️ Daily budget alert: ${daily_cost:.2f} / ${self.budget_alerts['daily_budget']:.2f}"
            )
        ...
        if weekly_cost > self.budget_alerts["weekly_budget"] * 0.8:
            logger.warning(
                f"⚠️ Weekly budget alert: ${weekly_cost:.2f} / ${self.budget_alerts['weekly_budget']:.2f}"
            )
    ...
    def get_cost_effectiveness_report(self, hours: int = 24) -> dict[str, Any]:
        ...
        for param_hash, records in param_groups.items():
            ...
            avg_cost_per_quality = sum(
                r["cost_per_quality_point"] for r in records
            ) / len(records)
        ...
        ranked_params = sorted(
            parameter_analysis.items(), key=lambda x: x[1]["avg_cost_per_quality_point"]
        )
        ...
        avg_quality = sum(r["quality_score"] for r in recent_records) / len(
            recent_records
        )
...
class GeminiMonitoringDashboard:
    ...
    def record_analysis_session(
        ...
        failure_indicators = self._identify_failure_indicators(
            response, quality_assessment
        )
        ...
        session_summary = {
            ...
            "cost_data": {
                "actual_cost": actual_cost,
                "cost_per_quality_point": actual_cost
                / max(0.1, quality_assessment["overall_quality"]),
            },
        ...
        logger.info(
            f"📊 Analysis session recorded: Quality={quality_assessment['overall_quality']:.2f}, Cost=${actual_cost:.4f}"
        )
    ...
    def _identify_success_indicators(
        self, response: str, quality_assessment: dict[str, Any]
    ) -> list[str]:
    ...
    def _identify_failure_indicators(
        ...
        if not any(
            word in response.lower() for word in ["recommend", "suggest", "advise"]
        ):
    ...
    def _assess_overall_performance(
        ...
        if (
            "error" in quality_trend
            or "error" in cost_effectiveness
            or not param_comparison.get("most_effective")
        ):
        ...
        cost_reasonable = (
            cost_effectiveness["avg_cost_per_request"] <= 0.50
        )  # $0.50 per request
        cost_efficient = (
            cost_effectiveness["cost_per_quality_point"] <= 1.0
        )  # $1 per quality point
    ...
    def _generate_improvement_recommendations(
        ...
        if param_comparison.get("most_effective"):
            ...
            recommendations.append(
                f"🎯 Use parameter set '{best_param_id}' - most cost-effective"
            )
        ...
        if "error" not in quality_trend:
            if quality_trend["averages"]["overall"] < 0.6:
                recommendations.append(
                    "📈 Quality below target - consider increasing max_tokens"
                )
            if quality_trend["averages"]["completeness"] < 0.7:
                recommendations.append(
                    "📝 Improve completeness - review prompt engineering"
                )
            if not quality_trend["trends"]["improving"]:
                recommendations.append(
                    "🔄 Quality not improving - review parameter adjustments"
                )
        ...
        if "error" not in cost_effectiveness:
            if cost_effectiveness["avg_cost_per_request"] > 0.75:
                recommendations.append(
                    "💰 High cost per request - consider more efficient parameters"
                )
            if cost_effectiveness["cost_per_quality_point"] > 1.5:
                recommendations.append(
                    "⚖️ Poor cost-quality ratio - optimize parameters"
                )

        # System recommendations
        if not recommendations:
            recommendations.append(
                "✅ System performing well - continue current configuration"
            )
    ...
    def _check_system_health(self) -> dict[str, Any]:
        ...
        health_status = {
            ...
            "parameter_tracking_active": len(self.parameter_tracker.effectiveness_data)
            > 0,
            "alerts_active": True,
        ...
        overall_health = all(
            health_status[key] for key in health_status if key != "last_health_check"
        )
...
if __name__ == "__main__":
    ...
    for i in range(3):
        ...
        print(
            f"Session {i+1}: Quality={session['quality_assessment']['overall_quality']:.2f}, Cost=${cost:.3f}"
        )

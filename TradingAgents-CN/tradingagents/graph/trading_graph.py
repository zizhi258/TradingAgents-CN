...
class TradingAgentsGraph:
    ...
    def _get_gemini_model_params(
        self, model_name: str, is_deep_thinking: bool = True
    ) -> dict[str, Any]:
        ...
        # Default parameters for unknown Gemini models
            ...
            logger.warning(
                f"⚠️ Unknown Gemini model '{model_name}', using default parameters"
            )
            if is_deep_thinking:
                return {
                    "temperature": 0.2,
    ...
    def __init__(
        ...
        if self.config["llm_provider"].lower() == "openai":
            ...
            try:
                ...
                deep_think_clean = {
                    k: v for k, v in deep_think_params.items() if not k.startswith("_")
                }
                quick_think_clean = {
                    k: v for k, v in quick_think_params.items() if not k.startswith("_")
                }

                logger.info(
                    f"🔧 [Enhanced Google AI] 深度思考模型: {self.config['deep_think_llm']}"
                )
                logger.info(f"   参数: {deep_think_clean}")
                if "_cost_monitoring" in deep_think_params:
                    cost_info = deep_think_params["_cost_monitoring"]
                    logger.info(
                        f"   估算成本: ${cost_info.get('estimated_cost_usd', 0):.4f}, 预算余量: {cost_info.get('remaining_budget_pct', 100):.1f}%"
                    )

                logger.info(
                    f"🔧 [Enhanced Google AI] 快速思考模型: {self.config['quick_think_llm']}"
                )
                ...
                logger.warning(
                    f"⚠️ Enhanced config failed, falling back to legacy parameters: {e}"
                )
                # Fallback to legacy method
                deep_think_clean = self._get_gemini_model_params(
                    self.config["deep_think_llm"], is_deep_thinking=True
                )
                quick_think_clean = self._get_gemini_model_params(
                    self.config["quick_think_llm"], is_deep_thinking=False
                )
        ...
        # DashScope/阿里百炼 已废弃，不再支持
            ...
            base_url = self.config.get("backend_url") or _os.getenv(
            ...
            logger.info("✅ [DeepSeek] 已启用token统计功能")
        else:
    ...
    def analyze_with_collaboration(
        self,
        company_name: str,
        trade_date: str,
    ...
    def _traditional_analysis_wrapper(
    ...
    def smart_analyze(
        self, company_name: str, trade_date: str, context: dict[str, Any] = None
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"智能分析选择: 模式={collaboration_mode}, 智能体={selected_agents}"
        )

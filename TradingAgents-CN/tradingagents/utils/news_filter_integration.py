...
def patch_akshare_utils():
    ...
    try:
        ...
        if not hasattr(akshare_utils, "_original_get_stock_news_em"):
            ...
            logger.info(
                "[新闻过滤集成] ✅ 成功为akshare_utils.get_stock_news_em添加过滤功能"
            )
        else:
            ...
            if any(
                ...
                try:
                    ...
                    if enable_filter and not original_news_df.empty:
                        ...
                        filtered_news_df = news_filter.filter_news(
                            original_news_df, min_score=min_score
                        )

                        # 记录过滤统计
                        filter_stats = news_filter.get_filter_statistics(
                            original_news_df, filtered_news_df
                        )
                        logger.info("[新闻过滤集成] 新闻过滤完成:")
                    ...
                    if not filtered_news_df.empty:
                        ...
                        logger.info(
                            f"[增强实时新闻] ✅ 成功生成过滤新闻报告，包含 {news_count} 条高质量新闻"
                        )
                        return report
                    else:
                        logger.warning(
                            "[增强实时新闻] 过滤后无符合条件的新闻，返回原始报告"
                        )
                        return original_report

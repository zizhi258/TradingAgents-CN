...
class PerformanceOptimizer:
    ...
    async def get_cached_chart(self, cache_key: str) -> dict[str, Any] | None:
        """获取缓存的图表"""
    ...
    async def cache_chart(
        self, cache_key: str, chart_data: dict[str, Any], ttl_hours: int = 24
    ) -> bool:
        """缓存图表数据"""
        if not self.cache_enabled:
            return False
    ...
    def _cache_to_memory(
        self, cache_key: str, data: dict[str, Any], ttl_hours: int = 1
    ):
        """缓存到内存"""
        try:
            with self.cache_lock:
    ...
    def _cleanup_memory_cache(self):
        ...
        with self.cache_lock:
            now = datetime.now()
            expired_keys = [
                key for key, (_, expiry) in self.memory_cache.items() if expiry < now
            ]
    ...
    async def optimize_chart_generation(
        self, chart_func: Callable, *args, **kwargs
    ) -> dict[str, Any]:
        """优化的图表生成方法"""
        ...
        try:
            ...
            result = await loop.run_in_executor(
                self.executor, self._execute_chart_generation, chart_func, args, kwargs
            )
            ...
            logger.info(
                f"Chart generated in {generation_time:.2f}s, cached with key: {cache_key}"
            )
            return result
    ...
    def _execute_chart_generation(
        self, chart_func: Callable, args: tuple, kwargs: dict
    ) -> dict[str, Any]:
        """在线程中执行图表生成"""
        try:
    ...
    def _check_system_resources(self) -> bool:
        ...
        try:
            ...
            # 检查磁盘空间
            charts_dir = Path(
                os.getenv("CHART_STORAGE_PATH", "data/attachments/charts")
            )
            ...
            if self.metrics.concurrent_jobs >= self.max_concurrent_jobs:
                logger.warning(
                    f"Max concurrent jobs reached: {self.metrics.concurrent_jobs}"
                )
                return False
    ...
    async def _queue_chart_generation(
        self, chart_func: Callable, *args, **kwargs
    ) -> dict[str, Any]:
        """将图表生成任务加入队列"""
        try:
    ...
    async def batch_optimize_charts(
        self, chart_requests: list[dict[str, Any]]
    ) -> list[dict[str, Any]]:
        """批量优化图表生成"""
        ...
        async def generate_with_semaphore(request):
            async with semaphore:
                return await self.optimize_chart_generation(
                    request["chart_func"], **request["kwargs"]
                )
        ...
        logger.info(
            f"Batch optimization completed: {len(successful_results)} success, {failed_count} failed"
        )
        return successful_results

    def _analyze_batch_requests(
        ...
        logger.info(
            f"Batch analysis: {len(requests)} -> {len(optimized_requests)} requests (removed {len(requests) - len(optimized_requests)} duplicates)"
        )
    ...
    def get_performance_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            self.metrics.memory_usage_mb = (
                (memory_info.total - memory_info.available) / 1024 / 1024
            )
            ...
            # 计算错误率
            total_operations = (
                self.metrics.total_charts_generated + self.metrics.error_count
            )
            error_rate = (
                (self.metrics.error_count / total_operations * 100)
...
class PerformanceMonitor:
    ...
    def get_metrics_summary(self, hours: int = 1) -> dict[str, Any]:
        ...
        return {
            ...
            "latest_chart_count": (
                recent_metrics[-1].get("chart_file_count", 0) if recent_metrics else 0
            ),
            "latest_disk_usage_mb": (
                recent_metrics[-1].get("disk_usage_mb", 0) if recent_metrics else 0
            ),
        }

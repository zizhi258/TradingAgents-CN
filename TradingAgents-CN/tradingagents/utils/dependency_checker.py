...
class DependencyChecker:
    ...
    def check_provider_dependencies(self, provider: str) -> DependencyResult:
        ...
        logger.debug(
            f"依赖检查 {provider}: {'可用' if is_available else '不可用'} - {error_msg}"
        )
        return result
    ...
    def check_all_providers(self) -> dict[str, DependencyResult]:
        ...
        logger.info(
            f"依赖检查完成 - 可用: {available_providers}, 不可用: {unavailable_providers}"
        )
            ...
            if provider in filtered_config:
                ...
                if result and not result.available:
                    ...
                    if isinstance(filtered_config[provider], dict):
                        ...
                        logger.warning(
                            f"由于依赖缺失，禁用了 {provider}: {result.error_message}"
                        )
                    else:
                        # 如果配置不是字典，则移除整个配置项
                        filtered_config.pop(provider, None)
                        logger.warning(
                            f"由于依赖缺失，移除了 {provider} 配置: {result.error_message}"
                        )
    ...
    def validate_config(self, config: dict[str, any]) -> tuple[bool, list[str]]:
        ...
        for provider in providers_to_check:
            if provider in config:
                ...
                if isinstance(provider_config, dict) and provider_config.get(
                    "enabled", False
                ):
                    result = dependency_results.get(provider)

...
from tradingagents.core.multi_model_manager import (
    MultiModelManager,
    TaskResult,
)
...
class AgentPerformanceTracker:
    ...
    def get_agent_score(self, agent_role: AgentRole) -> dict[str, float]:
        ...
        speed = 1.0 - (
            statistics.mean(metrics["response_time"]) / 60.0
        ...
        overall_score = (
            accuracy * 0.4 + speed * 0.2 + collaboration * 0.3 + calibration * 0.1
        )
...
class DynamicAgentSelector:
    ...
    async def select_agents_for_task(
        self, task: CoordinationTask, max_agents: int = 5
    ) -> list[AgentRole]:
        """
        ...
        try:
            ...
            if self.llm_orchestrator:
                selected_agents = await self._ai_agent_selection(
                    task, agent_scores, max_agents
                )
            else:
                selected_agents = self._heuristic_agent_selection(
                    task, agent_scores, max_agents
                )

            logger.info(
                f"Selected {len(selected_agents)} agents for task: {task.task_type}"
            )
    ...
    def _calculate_task_fitness(
        self, profile: AgentProfile, task: CoordinationTask
    ) -> float:
        ...
        if task.required_capabilities:
            ...
            if matching_capabilities:
                capability_score = sum(
                    cap.proficiency_level for cap in matching_capabilities
                ) / len(matching_capabilities)
        else:
            # Use average capability if no specific requirements
            capability_score = sum(
                cap.proficiency_level for cap in profile.capabilities
            ) / max(len(profile.capabilities), 1)
    ...
    async def _ai_agent_selection(
        ...
        for role, score in agent_scores.items():
            ...
            agent_info.append(
                f"- {role.value}: {profile.name} (fitness: {score:.2f}, authority: {profile.authority_level:.2f})"
            )
...
"""
    ...
    def _heuristic_agent_selection(
        ...
        if task.priority > 0.8:
            for role, score in sorted_agents:
                if (
                    self.agent_profiles[role].authority_level > 0.8
    ...
    def _is_complementary(
        ...
        return (
            len(new_domains) > 0
            or self.agent_profiles[candidate_role].authority_level > 0.8
        )
...
class ConsensusEngine:
    ...
    def _majority_vote_consensus(self, opinions: list[AgentOpinion]) -> ConsensusResult:
        ...
        majority_position = max(
            position_votes.keys(), key=lambda k: len(position_votes[k])
        )
        ...
        dissenting_opinions = [
            op for op in opinions if op.position != majority_position
        ]
    ...
    async def _weighted_average_consensus(
        self, opinions: list[AgentOpinion], context: dict[str, Any]
    ) -> ConsensusResult:
        ...
        for opinion in opinions:
            ...
            agent_authority = context.get("agent_authorities", {}).get(
                opinion.agent_role, 0.5
            )
        ...
        confidence = sum(
            op.confidence * w for op, w in zip(opinions, weights, strict=False)
        )
    ...
    async def _ai_synthesis(
        ...
        weighted_opinions = []
        for opinion, weight in zip(opinions, weights, strict=False):
...
"""
    ...
    def _simple_synthesis(
        self, opinions: list[AgentOpinion], weights: list[float]
    ) -> str:
        """Simple synthesis without AI"""

        # Find most common themes
        [op.position for op in opinions]
...
"""
    ...
    def _bayesian_fusion_consensus(
        ...
        weights = (
            [op.confidence / total_evidence for op in opinions]
            if total_evidence > 0
            else [1 / len(opinions)] * len(opinions)
        )

        # Create consensus decision
        final_decision = (
            "Bayesian fusion of expert opinions suggests a balanced approach."
        )
    ...
    def _expert_overrule_consensus(
        self, opinions: list[AgentOpinion]
    ) -> ConsensusResult:
...
class ConflictResolutionEngine:
    ...
    async def resolve_conflict(
        self,
        conflicting_opinions: list[AgentOpinion],
        ...
        try:
            ...
            if strategy == ConflictResolutionStrategy.DEBATE:
                ...
                return await self._resolve_through_compromise(
                    conflicting_opinions, context
                )

            elif strategy == ConflictResolutionStrategy.ADDITIONAL_EXPERT:
                return await self._resolve_additional_expert(
                    conflicting_opinions, context
                )
...
"""
    ...
    async def _resolve_evidence_based(
        ...
        for opinion in opinions:
            ...
            evidence_score = min(
                1.0, evidence_count / 3.0
            )  # Normalize to max 3 pieces of evidence
    ...
    def _resolve_authority_based(self, opinions: list[AgentOpinion]) -> ConsensusResult:
        ...
        return ConsensusResult(
            ...
            dissenting_opinions=[
                op for op in opinions if op != highest_authority_opinion
            ],
        )

    async def _resolve_through_compromise(
        self, opinions: list[AgentOpinion], context: dict[str, Any]
    ) -> ConsensusResult:
...
"""
        ...
        compromise_text = (
            "A balanced approach incorporating insights from all expert perspectives."
        )
    ...
    async def _resolve_additional_expert(
        self, opinions: list[AgentOpinion], context: dict[str, Any]
    ) -> ConsensusResult:
...
class EnhancedMultiAgentCoordinator:
    ...
    def __init__(self, multi_model_manager: MultiModelManager, llm_orchestrator=None):
    ...
    async def execute_coordinated_analysis(
        ...
        try:
            ...
            workflow_id = (
                f"workflow_{start_time.strftime('%Y%m%d_%H%M%S')}_{task.task_id}"
            )
            ...
            logger.info(
                f"Selected agents for {task.task_type}: {[agent.value for agent in selected_agents]}"
            )
            ...
            if (
                consensus_result.consensus_score < 0.6
                and consensus_result.dissenting_opinions
            ):
            ...
            await self._update_agent_performance(
                selected_agents, individual_results, final_result
            )
    ...
    async def _execute_individual_analyses(
        self, task: CoordinationTask, agents: list[AgentRole]
    ) -> dict[str, TaskResult]:
...
"""
            ...
            specialization_prompt = (
                f"Please analyze the following as a {agent.value}:\n\n{base_prompt}"
            )

        return specialization_prompt

    def _extract_opinions(
        self, results: dict[str, TaskResult], agents: list[AgentRole]
    ) -> list[AgentOpinion]:
    ...
    async def _update_agent_performance(
        ...
        for agent in agents:
            if agent.value in individual_results:
                ...
                collaboration_score = (
                    final_result.consensus_score
                )  # How well they aligned with consensus
                confidence_calibration = (
                    0.8  # Would need more sophisticated calculation
                )
    ...
    def get_coordination_stats(self) -> dict[str, Any]:
        ...
        successful_workflows = [
            w for w in self.coordination_history if w.get("success", False)
        ]
        ...
        avg_execution_time = statistics.mean(
            [w.get("execution_time", 0) for w in successful_workflows]
        )
        ...
        return {
            ...
            "avg_consensus_score": (
                statistics.mean(consensus_scores) if consensus_scores else 0.0
            ),
            "agent_performance": {
                agent.value: self.agent_selector.performance_tracker.get_agent_score(
                    agent
                )
...
def create_market_analysis_task(
    symbol: str, analysis_type: str = "comprehensive"
) -> CoordinationTask:
...
def create_risk_assessment_task(
    symbols: list[str], portfolio_context: dict[str, Any] = None
) -> CoordinationTask:

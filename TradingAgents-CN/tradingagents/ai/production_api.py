...
class RequestModels:
    ...
    class AnalysisRequest(BaseModel):
        ...
        agent_role: str | None = Field(default=None, description="Specific agent role")
        priority: PriorityLevel = Field(
            default=PriorityLevel.NORMAL, description="Request priority"
        )
    ...
    class MultiSymbolAnalysisRequest(BaseModel):
        ...
        coordination_mode: str = Field(
            default="parallel", description="Coordination mode"
        )
    ...
    class RAGQueryRequest(BaseModel):
        ...
        symbols: list[str] | None = Field(default=None, description="Related symbols")
        max_documents: int = Field(
            default=5, description="Maximum documents to retrieve"
        )
        relevance_threshold: float = Field(
            default=0.7, description="Relevance threshold"
        )
...
class RateLimiter:
    ...
    async def check_rate_limit(
        self, client_id: str, priority: PriorityLevel
    ) -> dict[str, Any]:
        """
...
class CostOptimizer:
    ...
    def estimate_cost(
        self, request_type: str, priority: PriorityLevel, context: dict[str, Any]
    ) -> float:
    ...
    def check_cost_limits(
        self, client_id: str, estimated_cost: float
    ) -> dict[str, Any]:
    ...
    def record_cost(self, client_id: str, actual_cost: float):
        ...
        self.cost_history.append(
            {"client_id": client_id, "cost": actual_cost, "timestamp": current_time}
        )
...
class ProductionAPIServer:
    ...
    def set_ai_components(
        self, ai_orchestrator, rag_system, automation_system, coordinator
    ):
    ...
    def _setup_routes(self):
        ...
        async def health_check():
            ...
            return ResponseModels.SystemStatusResponse(
                success=True, data=health_status, metadata={"uptime": uptime}
            )
        ...
        async def get_metrics():
            ...
            if self.response_times:
                ...
                self.metrics.requests_per_minute = len(
                    [rt for rt in self.response_times if time.time() - rt < 60]
                )

            self.metrics.error_rate = self.metrics.failed_requests / max(
                self.metrics.total_requests, 1
            )

            self.metrics.last_updated = datetime.now()

            return ResponseModels.APIResponse(success=True, data=asdict(self.metrics))

        @self.app.post("/analysis", response_model=ResponseModels.AnalysisResponse)
        async def analyze_symbol(
            request: RequestModels.AnalysisRequest,
            ...
            try:
                ...
                estimated_cost = self.cost_optimizer.estimate_cost(
                    "analysis",
                    request.priority,
                    {"symbol": request.symbol, "use_cache": request.use_cache},
                )

                cost_check = self.cost_optimizer.check_cost_limits(
                    client_id, estimated_cost
                )
                ...
                actual_cost = (
                    result.actual_cost
                    if hasattr(result, "actual_cost")
                    else estimated_cost
                )
                bg_tasks.add_task(
                    self.cost_optimizer.record_cost, client_id, actual_cost
                )
                ...
                return ResponseModels.AnalysisResponse(
                    ...
                    error=(
                        {
                            "code": APIErrorCode.AI_SERVICE_ERROR.value,
        ...
        async def rag_query(
            request: RequestModels.RAGQueryRequest,
            bg_tasks: BackgroundTasks,
            client_id: str = Depends(self._get_client_id),
        ):
            """Execute RAG query"""
            ...
            try:
                # Rate limiting
                rate_check = await self.rate_limiter.check_rate_limit(
                    client_id, PriorityLevel.NORMAL
                )
                if not rate_check["allowed"]:
                ...
                # Cost check
                estimated_cost = self.cost_optimizer.estimate_cost(
                    "rag_query", PriorityLevel.NORMAL, {}
                )
                cost_check = self.cost_optimizer.check_cost_limits(
                    client_id, estimated_cost
                )
        ...
        async def generate_report(
            request: RequestModels.ReportGenerationRequest,
            bg_tasks: BackgroundTasks,
            client_id: str = Depends(self._get_client_id),
        ):
            ...
            try:
                ...
                report_type = report_type_map.get(
                    request.report_type, ReportType.DAILY_SUMMARY
                )

                report_result = (
                    await self.automation_system.report_generator.generate_report(
                ...
                return ResponseModels.APIResponse(
                    success=True, data=report_result, cost=actual_cost
                )
        ...
        async def multi_symbol_analysis(
            request: RequestModels.MultiSymbolAnalysisRequest,
            bg_tasks: BackgroundTasks,
            client_id: str = Depends(self._get_client_id),
        ):
            ...
            try:
                ...
                from tradingagents.ai.enhanced_coordination import (
                    CoordinationMode,
                    CoordinationTask,
                )
                ...
                coordination_mode = mode_map.get(
                    request.coordination_mode, CoordinationMode.PARALLEL
                )
                ...
                return ResponseModels.APIResponse(
                    success=result.get("success", False), data=result, cost=actual_cost
                )
...
async def test_api_endpoints():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post(f"{base_url}/analysis", json=analysis_request)

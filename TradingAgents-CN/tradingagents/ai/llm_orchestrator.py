...
class IntelligentCache:
    ...
    def _generate_key(
        self, agent_role: str, prompt: str, context: dict[str, Any]
    ) -> str:
    ...
    def put(
        self, agent_role: str, prompt: str, context: dict[str, Any], result: TaskResult
    ):
    ...
    def _is_stale_financial_data(
        self, current_context: dict[str, Any], cache_entry: dict[str, Any]
    ) -> bool:
...
class AIOrchestrator:
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.circuit_breaker_config = config.get("routing", {}).get(
            "circuit_breaker", {}
        )
    ...
    async def execute_collaborative_task(
        ...
        logger.info(
            f"Starting collaborative task: {collaboration_mode} with {len(participating_agents)} agents"
        )
        ...
        try:
            ...
            if self._is_circuit_open(selected_model):
                ...
                fallback_models = self._get_fallback_models(
                    task.agent_role, task.task_type
                )
            ...
            self._record_failure(
                selected_model if "selected_model" in locals() else "unknown"
            )
    ...
    async def _intelligent_model_selection(self, task: AITask) -> str | None:
    ...
    def _check_cost_limits(self) -> bool:
        ...
        if self.daily_cost >= daily_max:
            logger.warning(
                f"Daily cost limit exceeded: ${self.daily_cost:.2f} >= ${daily_max:.2f}"
            )
            return False

        if self.session_cost >= session_max:
            logger.warning(
                f"Session cost limit exceeded: ${self.session_cost:.2f} >= ${session_max:.2f}"
            )
            return False
    ...
    def _is_circuit_open(self, model_name: str) -> bool:
        ...
        if failure_count >= failure_threshold:
            if (
                last_failure
                and (datetime.now() - last_failure).seconds < recovery_timeout
            ):
                return True
            else:
    ...
    def get_metrics(self) -> dict[str, Any]:
        ...
        return {
            "performance": {
                ...
                / max(1, self.metrics.total_tasks),
    ...
    def get_health_status(self) -> dict[str, Any]:
        ...
        orchestrator_health = {
            ...
            "cost_status": {
                "daily_usage_pct": (
                    self.daily_cost / self.cost_thresholds.get("daily_max", 10.0)
                )
                * 100,
                "session_usage_pct": (
                    self.session_cost / self.cost_thresholds.get("session_max", 1.0)
                )
                * 100,
    ...
    def _get_health_recommendations(self) -> list[str]:
        ...
        if self.daily_cost > self.cost_thresholds.get("daily_max", 10.0) * 0.8:
            recommendations.append(
                "Daily cost approaching limit - consider optimizing model selection"
            )

        # Cache recommendations
        if self.cache.hit_rate < 0.3:
            recommendations.append(
                "Low cache hit rate - consider adjusting cache TTL or task patterns"
            )
        ...
        if open_circuits > 0:
            recommendations.append(
                f"{open_circuits} models are circuit-broken - check model health"
            )

        # Performance recommendations
        if self.metrics.average_response_time > 30:
            recommendations.append(
                "High response times detected - consider increasing concurrency"
            )

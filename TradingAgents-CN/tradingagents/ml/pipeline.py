...
class MLPipeline:
    ...
    def __init__(self, config: PipelineConfig):
        ...
        self.inference_service = InferenceService(
            config.deployment_config, self.model_registry
        )
    ...
    def run_full_pipeline(self, symbols: list[str] | None = None) -> dict:
        ...
        for symbol in symbols:
            try:
                ...
                if self.config.market_type == "US":
                    ...
                    logger.warning(
                        f"Market type {self.config.market_type} not fully supported"
                    )
                    continue
                ...
                logger.info(
                    f"Data ingestion completed for {symbol}: {len(market_data)} records"
                )
    ...
    def _assess_data_quality(self, data: pd.DataFrame) -> dict:
        ...
        quality_metrics = {
            ...
            "zero_volume_days": (
                (data["Volume"] == 0).sum() if "Volume" in data.columns else 0
            ),
            "price_anomalies": {
                "negative_prices": (
                    (data["Close"] <= 0).sum() if "Close" in data.columns else 0
                ),
    ...
    def _engineer_features(self, data_results: dict) -> dict:
        ...
        feature_results = {"features": {}, "feature_names": [], "feature_stats": {}}
        ...
        for symbol, market_data in data_results["market_data"].items():
            try:
                ...
                if features is not None and not features.empty:
                    ...
                    logger.info(
                        f"Features engineered for {symbol}: {len(features.columns)} features, {len(features)} samples"
                    )
                else:
    ...
    def _train_models(self, feature_results: dict) -> dict:
        ...
        training_results = {"models": {}, "training_metrics": {}, "model_metadata": {}}
            ...
            for symbol in feature_results["features"]:
                try:
                    ...
                    if len(features) < self.config.min_training_samples:
                        logger.warning(
                            f"Insufficient data for {symbol}: {len(features)} < {self.config.min_training_samples}"
                        )
                        continue

                    # Create target variable
                    target = self._create_target_variable(
                        features, task_name, model_config
                    )

                    if target is None or len(target) == 0:
                        logger.warning(
                            f"Could not create target variable for {task_name} - {symbol}"
                        )
                        continue
                    ...
                    if len(aligned_features) < self.config.min_training_samples:
                        logger.warning(
                            f"Insufficient aligned data for {symbol}: {len(aligned_features)}"
                        )
                        continue

                    # Create and train model
                    model = create_model(
                        model_config.model_type, task_name, **asdict(model_config)
                    )
                    ...
                    if task_name == "price_prediction":
                        ...
                        training_history = model.train(
                            aligned_features, returns, volatility
                        )
                    else:
                        training_history = model.train(aligned_features, aligned_target)
                    ...
                    logger.error(
                        f"Model training failed for {task_name} - {symbol}: {e}"
                    )
                    continue
    ...
    def _create_target_variable(
        self, features: pd.DataFrame, task_name: str, model_config: ModelConfig
    ) -> pd.Series | None:
        ...
        try:
            ...
            if not price_columns:
                ...
                price_columns = [
                    ...
                    if any(word in col.lower() for word in ["price", "close"])
                ]
            ...
            if len(price_columns) > 0:
                ...
                logger.warning(
                    "Using feature-based price reconstruction - consider providing actual price data"
                )
                return None
            ...
            if task_name == "price_prediction":
                ...
                return pd.Series(
                    dummy_returns, index=features.index, name="future_returns"
                )
                ...
                return pd.Series(
                    dummy_volatility, index=features.index, name="volatility"
                )
        ...
        for task_name, task_models in training_results["models"].items():
            ...
            for symbol, model in task_models.items():
                try:
                    ...
                    features = data_results["market_data"][
                        symbol
                    ]  # Use original market data for validation
                    ...
                    if target is None:
                        logger.warning(
                            f"Could not create validation target for {task_name} - {symbol}"
                        )
                        continue

                    # Align data
                    aligned_features, aligned_target = simple_features.align(
                        target, join="inner"
                    )
                    ...
                    task_type = (
                        "regression"
                        if task_name in ["price_prediction", "volatility_prediction"]
                        else "classification"
                    )
                    ...
                    validation_result = run_comprehensive_validation(
                        ...
                        returns=(
                            features["Close"].pct_change().dropna()
                    ...
                    logger.error(
                        f"Model validation failed for {task_name} - {symbol}: {e}"
                    )
                    continue
    ...
    def _create_validation_target(
        self, data: pd.DataFrame, task_name: str
    ) -> pd.Series | None:
        ...
        for task_name, task_models in training_results["models"].items():
            ...
            for symbol, model in task_models.items():
                try:
                    ...
                    validation_result = (
                        validation_results.get("cross_validation", {})
                        .get(task_name, {})
                        .get(symbol)
                    )
                    ...
                    if validation_result:
                        ...
                        if deploy_model_flag:
                            ...
                            logger.warning(
                                f"Model failed deployment criteria: {task_name} - {symbol}"
                            )
                    else:
                        logger.warning(
                            f"No validation results for deployment: {task_name} - {symbol}"
                        )

                except Exception as e:
                    logger.error(
                        f"Model deployment failed for {task_name} - {symbol}: {e}"
                    )
                    continue
    ...
    def get_pipeline_status(self) -> dict:
        ...
        return {
            ...
            "successful_runs": len(
                [run for run in self.pipeline_runs if run.get("status") == "completed"]
            ),
            "failed_runs": len(
                [run for run in self.pipeline_runs if run.get("status") == "failed"]
            ),
            ...
            "inference_service_status": (
                "active" if self.inference_service else "inactive"
            ),
        }
        ...
        for task_name in self.trained_models:
            try:
                ...
                if model_key in self.inference_service.loaded_models:
                    result = self.inference_service.predict(model_key, features.values)
...
def quick_model_training(
    symbol: str, model_type: str = "xgboost", task: str = "price_prediction"
) -> dict:
    ...
    pipeline.config.model_configs = {task: ModelConfig(model_type=model_type)}

...
from .pipeline import MLPipeline, create_default_pipeline
...
class MLEnhancedAnalyst(BaseSpecializedAgent):
    ...
    async def analyze_stock(
        ...
        try:
            ...
            combined_analysis = self._combine_analyses(
                traditional_analysis, ml_analysis, symbol
            )
    ...
    async def _get_market_data(self, symbol: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            if data is None or data.empty:
                # Try China market
                data = get_china_stock_data_unified(
    ...
    async def _traditional_analysis(self, data: pd.DataFrame, symbol: str) -> dict:
        ...
        try:
            ...
            analysis.update(
                {
                    ...
                    * 100,
                    ...
                    "high_52w": (
                        data["High"].tail(252).max()
                        if len(data) >= 252
                        else data["High"].max()
                    ),
                    "low_52w": (
                        data["Low"].tail(252).min()
                        if len(data) >= 252
                        else data["Low"].min()
                    ),
                }
            ...
            analysis["technical_indicators"] = self._calculate_technical_indicators(
                data
            )
    ...
    async def _ml_analysis(self, data: pd.DataFrame, symbol: str) -> dict:
        ...
        try:
            ...
            for task in tasks:
                try:
                    prediction_result = self.ml_pipeline.get_model_predictions(
                        symbol, features.tail(1)
                    )

                    if task in prediction_result and prediction_result[task].get(
                        "success"
                    ):
                        analysis["predictions"][task] = prediction_result[task][
                            "prediction"
                        ]
                        analysis["confidence_scores"][task] = (
                            self._calculate_confidence(prediction_result[task])
                        )
                        analysis["model_info"][task] = {
                            "model_type": prediction_result[task].get("model_type"),
                            "model_version": prediction_result[task].get(
                                "model_version"
                            ),
            ...
            analysis["ml_insights"] = self._generate_ml_insights(
    ...
    async def _generate_features(self, data: pd.DataFrame) -> pd.DataFrame | None:
        ...
        try:
            ...
            if price_pred is not None:
                if isinstance(price_pred, list) and len(price_pred) > 0:
                    ...
                    if price_change > 0.02:  # 2% threshold
                        ...
                        insights["price_confidence"] = (
                            "high" if price_change > 0.05 else "medium"
                        )
            ...
            if signal_pred is not None:
                if isinstance(signal_pred, list) and len(signal_pred) > 0:
                    ...
                    insights["signal_strength"] = (
                        "strong" if signal in ["BUY", "SELL"] else "weak"
                    )
                ...
                if isinstance(risk_pred, list) and len(risk_pred) > 0:
                    ...
                    if risk_level == "HIGH":
                        insights["risk_warning"] = (
                            "High risk detected - consider position sizing"
                        )
                    elif risk_level == "LOW":
                        insights["risk_note"] = (
                            "Low risk environment - potential opportunity"
                        )
    ...
    def _combine_analyses(
        self, traditional: dict, ml_analysis: dict, symbol: str
    ) -> dict:
        ...
        try:
            ...
            avg_ml_confidence = (
                np.mean(list(ml_confidences.values())) if ml_confidences else 0.0
            )

            # Weight traditional analysis confidence (simplified)
            traditional_confidence = (
                0.7  # Assume 70% confidence for traditional analysis
            )
    ...
    def _generate_combined_insights(self, traditional: dict, ml_analysis: dict) -> dict:
        ...
        try:
            ...
            traditional_trend = traditional.get("trend_analysis", {}).get(
                "overall_trend", "neutral"
            )
            ml_outlook = ml_analysis.get("ml_insights", {}).get(
            ...
            traditional_volatility = traditional.get("technical_indicators", {}).get(
                "volatility", "medium"
            )
            ...
            traditional_momentum = traditional.get("technical_indicators", {}).get(
                "momentum", "neutral"
            )
    ...
    def _generate_recommendations(
        self, traditional: dict, ml_analysis: dict, confidence: float
    ) -> dict:
        ...
        try:
            ...
            price_outlook = ml_analysis.get("ml_insights", {}).get(
                "price_outlook", "neutral"
            )
    ...
    def _calculate_technical_indicators(self, data: pd.DataFrame) -> dict:
        ...
        try:
            ...
            indicators["price_vs_sma50"] = (
                "above" if current_price > sma_50 else "below"
            )
            ...
            # Momentum
            if len(data) >= 10:
                momentum = (current_price - data["Close"].iloc[-10]) / data[
                    "Close"
                ].iloc[-10]
                if momentum > 0.05:
        ...
        try:
            ...
            if len(data) >= 20:
                ...
                trend_analysis["short_term"] = (
                    "bullish"
                    if short_trend_pct > 0.02
                    else "bearish" if short_trend_pct < -0.02 else "neutral"
                )
            ...
            if len(data) >= 60:
                ...
                trend_analysis["medium_term"] = (
                    "bullish"
                    if medium_trend_pct > 0.05
                    else "bearish" if medium_trend_pct < -0.05 else "neutral"
                )
    ...
    def _find_support_resistance(self, data: pd.DataFrame) -> dict:
        ...
        try:
            ...
            if len(data) >= 50:
                ...
                levels["distance_to_resistance"] = (
                    resistance - current_price
                ) / current_price
                levels["distance_to_support"] = (
...
class MLEnhancedChartingArtist(BaseSpecializedAgent):
    ...
    async def create_ml_enhanced_chart(
        self, symbol: str, chart_type: str = "price_prediction"
    ) -> dict:
        ...
        try:
            ...
            predictions = (
                self.ml_pipeline.get_model_predictions(symbol, features.tail(30))
            ...
            chart_data = {
                ...
                "market_data": {
                    ...
                    "dates": [
                        d.strftime("%Y-%m-%d") for d in market_data.index.tail(100)
                    ],
                },
                "ml_overlays": {},
            ...
            if chart_type == "price_prediction":
                chart_data["ml_overlays"] = await self._create_prediction_overlay(
                    market_data, predictions
                )
            elif chart_type == "volatility_analysis":
                chart_data["ml_overlays"] = await self._create_volatility_overlay(
                    market_data, predictions
                )
            elif chart_type == "risk_assessment":
                chart_data["ml_overlays"] = await self._create_risk_overlay(
                    market_data, predictions
                )

            # Generate chart insights
            chart_data["chart_insights"] = self._generate_chart_insights(
                market_data, predictions
            )
    ...
    async def _create_prediction_overlay(
        self, data: pd.DataFrame, predictions: dict
    ) -> dict:
        ...
        try:
            ...
            if price_pred.get("success") and price_pred.get("prediction"):
                ...
                overlay["confidence_bands"] = {"upper": upper_band, "lower": lower_band}
    ...
    async def _create_volatility_overlay(
        self, data: pd.DataFrame, predictions: dict
    ) -> dict:
        ...
        try:
            ...
            if vol_pred.get("success") and vol_pred.get("prediction"):
                ...
                # Volatility zones
                current_vol = (
                    hist_vol.iloc[-1] if not pd.isna(hist_vol.iloc[-1]) else 0.2
                )
        ...
        try:
            ...
            if risk_pred.get("success") and risk_pred.get("prediction"):
                ...
                overlay["risk_levels"].append(
                    {"level": risk_level, "timestamp": datetime.now().isoformat()}
                )

                # Position sizing suggestions based on risk
                if risk_level == "HIGH":
                    overlay["position_sizing_suggestions"].append(
                        "Reduce position size by 50%"
                    )
                    overlay["risk_indicators"].append("high_risk_warning")
                elif risk_level == "LOW":
                    overlay["position_sizing_suggestions"].append(
                        "Normal position size acceptable"
                    )
...
def create_ml_enhanced_agents(
    ml_pipeline: MLPipeline, config: MLAgentConfig = None
) -> dict[str, BaseSpecializedAgent]:
    ...
    agents = {
        ...
        "ml_charting_artist": MLEnhancedChartingArtist(
            "ml_enhanced_charting_artist", ml_pipeline, config
        ),
    }

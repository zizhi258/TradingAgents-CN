...
class ModelRegistry:
    ...
    def get_model(
        ...
        if metadata_dict.get("deployed_at"):
            metadata_dict["deployed_at"] = datetime.fromisoformat(
                metadata_dict["deployed_at"]
            )
...
class ABTestingFramework:
    ...
    def start_ab_test(
        ...
        ab_test = {
            ...
            + timedelta(hours=self.config.ab_test_duration_hours),
            "status": "active",
            "metrics": {
                "model_a": {
                    "requests": 0,
    ...
    def route_traffic(self, test_id: str, request_id: str | None = None) -> str:
    ...
    def get_test_status(self, test_id: str) -> dict:
        ...
        status = {
            ...
            "time_remaining": max(
                0, (test["end_time"] - datetime.now()).total_seconds()
            ),
            "model_a": {
                "name": test["model_a"],
                ...
                "avg_latency": (
                    np.mean(metrics_a["latency"]) if metrics_a["latency"] else 0
                ),
            },
            "model_b": {
                "name": test["model_b"],
                ...
                "avg_latency": (
                    np.mean(metrics_b["latency"]) if metrics_b["latency"] else 0
                ),
            },
        }
    ...
    def _finalize_test(self, test_id: str) -> dict:
        ...
        if (
            metrics_a["requests"] >= self.config.ab_test_min_samples
            and metrics_b["requests"] >= self.config.ab_test_min_samples
        ):
...
class InferenceService:
    ...
    def load_model(self, model_name: str, version: str | None = None) -> str:
    ...
    def predict(
        self, model_key: str, features: np.ndarray, use_cache: bool = True
    ) -> dict:
    ...
    def _get_cached_prediction(self, cache_key: str) -> dict | None:
        ...
        if self.redis_client:
            try:
                self.redis_client.setex(
                    cache_key, self.config.cache_ttl, json.dumps(result, default=str)
                )
                return
            except Exception as e:
    ...
    def start_api_server(self) -> None:
        ...
        logger.info(
            f"Starting inference API server on {self.config.api_host}:{self.config.api_port}"
        )
        uvicorn.run(
            self.app,
...
class ModelMonitor:
    ...
    def get_model_metrics(self, model_name: str, hours: int = 24) -> dict:
        ...
        recent_metrics = [
            m for m in self.metrics_history[model_name] if m["timestamp"] > cutoff_time
        ]
...
def deploy_model(
    model: Any, metadata: ModelMetadata, config: DeploymentConfig | None = None
) -> dict:

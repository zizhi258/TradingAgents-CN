    ...
    try:
        ...
        return {
            ...
            "mongodb_url": os.getenv(
                "TRADINGAGENTS_MONGODB_URL", "mongodb://localhost:27017"
            ),
        ...
        for symbol in symbols:
            try:
                ...
                daily_data = adapter.get_daily_data(
                    ts_code=symbol, start_date=date_str, end_date=date_str
                )
...
def validate_extracted_data(**context) -> dict[str, Any]:
    ...
    try:
        ...
        # Run validation
        validation_results = validator.validate_market_data(
            data, data_type="daily_ohlc"
        )
        ...
        logging.info(
            f"Critical Failures: {validation_results['summary']['critical_failures']}"
        )
    ...
    try:
        ...
        for record in raw_data:
            try:
                ...
                logging.warning(
                    f"Failed to transform record {record.get('symbol', 'unknown')}: {e}"
                )
                continue
...
def load_to_mongodb(**context) -> dict[str, Any]:
    ...
    try:
        from tradingagents.dataflows.enhanced_mongodb_integration import (
            EnhancedMongoDBManager,
            MongoDBConfig,
        )
        ...
        mongodb_manager = EnhancedMongoDBManager(
            MongoDBConfig(connection_string=config["mongodb_url"])
        )

        # Load market data (rename: metadata -> data) using async API
        load_results = asyncio.run(mongodb_manager.store_market_data(data))

        logging.info(f"MongoDB load results: {load_results}")

        # Store results
        context["task_instance"].xcom_push(
            key="mongodb_load_results", value=load_results
        )
...
def load_to_redis_cache(**context) -> dict[str, Any]:
    ...
    try:
        from tradingagents.dataflows.enhanced_redis_integration import (
        ...
        for record in data:
            try:
                ...
                success = asyncio.run(
                    redis_manager.set_market_data(symbol, market_data, ttl=86400)
                )
                if success:
                ...
                logging.warning(
                    f"Failed to cache data for {record.get('symbol', 'unknown')}: {e}"
                )
                cache_results["errors"] += 1
    ...
    try:
        ...
        for record in data:
            try:
                ...
                success = asyncio.run(
                    redis_manager.store_ml_features(
                        symbol=symbol,
                        features=features,
                ...
                logging.warning(
                    f"Failed to process features for {record.get('symbol', 'unknown')}: {e}"
                )
                feature_results["errors"] += 1
...
def send_completion_notification(**context) -> None:
    ...
    try:
        ...
        mongodb_results = context["task_instance"].xcom_pull(
            key="mongodb_load_results", task_ids="load_to_mongodb"
        )
        ...
        message = {
            ...
            "status": "completed",
            "data_quality_score": (
                validation_results.get("data_quality_score", 0)
                if validation_results
                else 0
            ),
            "records_processed": (
                validation_results.get("total_records", 0) if validation_results else 0
            ),
            "mongodb_inserted": (
                mongodb_results.get("inserted", 0) if mongodb_results else 0
            ),
            "mongodb_updated": (
                mongodb_results.get("updated", 0) if mongodb_results else 0
            ),
        ...
        logging.info(
            f"Pipeline completion notification: {json.dumps(message, indent=2)}"
        )
...
with daily_market_dag:
    ...
    start_task = DummyOperator(task_id="start_pipeline", dag=daily_market_dag)
    ...
    end_task = DummyOperator(task_id="end_pipeline", dag=daily_market_dag)
...
def extract_financial_statements(**context) -> str:
    ...
    try:
        ...
        # Get symbol list
        symbols = json.loads(
            Variable.get("financial_processing_symbols", default_var="[]")
        )
        if not symbols:
        ...
        for symbol in symbols:
            try:
                ...
                income_data = adapter.get_income_statement(
                    ts_code=symbol, period="20231231"
                )
                ...
                # Get balance sheet
                balance_data = adapter.get_balance_sheet(
                    ts_code=symbol, period="20231231"
                )
...
def retrain_ml_models(**context) -> dict[str, Any]:
    ...
    try:
        ...
        # Store results
        results_file = (
            f"/tmp/model_training_results_{execution_date.strftime('%Y%m%d')}.json"
        )
        with open(results_file, "w") as f:

...
class TushareProvider:
    ...
    def get_stock_daily_probar(self, symbol: str, start_date: str = None, end_date: str = None, adj: str = 'qfq') -> pd.DataFrame:
        ...
        Args:
            ...
            adj: 复权类型('qfq'前复权, 'hfq'后复权, ''未复权)

        Returns:
            DataFrame: 复权后的日线数据
        """
        if not self.connected:
            logger.error(f"❌ [Tushare详细日志] Tushare未连接，无法获取 pro_bar 数据")
            return pd.DataFrame()
        ...
        try:
            ...
            logger.info(f"🔍 [Tushare详细日志] 调用 ts.pro_bar: ts_code='{ts_code}', start='{start_date_fmt}', end='{end_date_fmt}', adj='{adj}'")

            # 官方 pro_bar（与daily_basic不同，为bar端点）
            import tushare as ts
    ...
    def get_stock_info(self, symbol: str) -> Dict:
        ...
        if os.path.exists(p):
            try:
                ...
                logger.warning(f"⚠️ 读取本地info失败: {p}: {e}")
        sym = self._norm_symbol(symbol)
        return {"symbol": sym, "name": f"股票{sym}", "source": "local"}
    ...
    def get_financial_data(self, symbol: str, period: str = "20231231") -> Dict:
        ...
        if os.path.exists(p):
            try:
                ...
                logger.warning(f"⚠️ 读取本地financials失败: {p}: {e}")
        return {}

...
"""
    ...
    def _get_industry_info(self, symbol: str) -> dict:
        ...
        info = industry_map.get(
            code_prefix, {"industry": "其他", "market": "未知市场", "type": "综合"}
        )
    ...
    def _estimate_financial_metrics(self, symbol: str, current_price: str) -> dict:
        ...
        for key in estimated_metrics:
            if isinstance(estimated_metrics[key], str) and key not in [
                "fundamental_score",
                "valuation_score",
                "growth_score",
                "risk_level",
    ...
    def _parse_akshare_financial_data(
        ...
        try:
            ...
            if eps_value is not None and str(eps_value) != "nan" and eps_value != "--":
                try:
                    ...
                    if eps_val > 0:
                        ...
                        logger.debug(
                            f"✅ 计算PE: 股价{price_value} / EPS{eps_val} = {metrics['pe']}"
                        )
                    else:
                        metrics["pe"] = "N/A（亏损）"
                except (ValueError, TypeError):
            ...
            if bps_value is not None and str(bps_value) != "nan" and bps_value != "--":
                try:
                    ...
                    if bps_val > 0:
                        ...
                        logger.debug(
                            f"✅ 计算PB: 股价{price_value} / BPS{bps_val} = {metrics['pb']}"
                        )
                    else:
                        metrics["pb"] = "N/A"
                except (ValueError, TypeError):
            ...
            # 补充其他指标的默认值
            metrics.update(
                {"ps": "待计算", "dividend_yield": "待查询", "cash_ratio": "待分析"}
            )

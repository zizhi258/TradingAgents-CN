...
from pymongo import ASCENDING, DESCENDING, GEO2D, TEXT, MongoClient
...
class MongoDBConfig:
    ...
    write_concern_w: int | str = "majority"
        ...
        self.collection_configs = {
            DataCategory.MARKET_DATA: {
                ...
                "indexes": [
                    ...
                    IndexModel(
                        [("timestamp", DESCENDING), ("symbol", ASCENDING)],
            ...
            DataCategory.NEWS_DATA: {
                ...
                "indexes": [
                    IndexModel(
                        [("published_at", DESCENDING), ("symbols", ASCENDING)],
                    ...
                    IndexModel(
                        [("title", TEXT), ("content", TEXT)], name="text_search"
                    ),
                    ...
                    IndexModel(
                        [("sentiment.score", DESCENDING), ("published_at", DESCENDING)],
                        name="sentiment_published",
                        sparse=True,
                    ),
                ],
                "validator": {
            ...
            DataCategory.ANALYSIS_RESULTS: {
                ...
                "indexes": [
                    ...
                    IndexModel(
                        [("agent_name", ASCENDING), ("created_at", DESCENDING)],
                        name="agent_created",
                    ),
                    # Performance tracking indexes
                    IndexModel(
                        [("execution_time_ms", DESCENDING)], name="execution_time"
                    ),
                    IndexModel(
            ...
            DataCategory.FINANCIAL_STATEMENTS: {
                ...
                "indexes": [
                    ...
                    IndexModel(
                        [("period", DESCENDING), ("statement_type", ASCENDING)],
                        name="period_type",
                    ),
            ...
            for category, config in self.collection_configs.items():
                ...
                try:
                    ...
                    if "already exists" not in str(e).lower():
                        self.logger.warning(
                            f"Collection {collection_name} creation warning: {e}"
                        )
                ...
                try:
                    if config.get("indexes"):
                        ...
                        self.logger.info(
                            f"Created {len(config['indexes'])} indexes for {collection_name}"
                        )
                except Exception as e:
                    self.logger.error(
                        f"Index creation failed for {collection_name}: {e}"
                    )
...
class EnhancedMongoDBManager:
    ...
    async def store_market_data(
        ...
        try:
            collection = self.db[
                self.collection_manager.collection_configs[DataCategory.MARKET_DATA][
                    "name"
                ]
            ]
                    ...
                    try:
                        ...
                        for error in e.details.get("writeErrors", []):
                            results["errors"].append(
                                {
                                    "error": error.get(
                                        "errmsg", "Unknown bulk write error"
                                    ),
                                    "code": error.get("code"),
                                }
                            )
    ...
    def _prepare_market_data_document(self, record: dict[str, Any]) -> dict[str, Any]:
        ...
        document = {
            ...
            "market": record.get(
                "market", self._infer_market_from_symbol(record["symbol"])
            ),
    ...
    async def store_news_articles(
        ...
        try:
            collection = self.db[
                self.collection_manager.collection_configs[DataCategory.NEWS_DATA][
                    "name"
                ]
            ]
            ...
            for article in articles:
                try:
                    ...
                    results["errors"].append({"article": article, "error": str(e)})
    ...
    async def store_analysis_results(
        ...
        try:
            collection = self.db[
                self.collection_manager.collection_configs[
                    DataCategory.ANALYSIS_RESULTS
                ]["name"]
            ]

            storage_results = {"inserted": 0, "errors": []}
        ...
        for field in optional_fields:
            if field in result:
                if field in ["confidence_score", "execution_time_ms", "cost"]:
                    document[field] = (
                        float(result[field]) if result[field] is not None else None
                    )
                elif field == "tokens_used":
                    document[field] = (
                        int(result[field]) if result[field] is not None else None
                    )
                else:
    ...
    async def get_latest_prices(self, symbols: list[str]) -> dict[str, dict[str, Any]]:
        ...
        try:
            ...
            pipeline = [
                ...
                {"$group": {"_id": "$symbol", "latest_data": {"$first": "$$ROOT"}}},
    ...
    async def store_file(
        self, filename: str, data: bytes, metadata: dict[str, Any] = None
    ) -> str:
    ...
    async def cleanup_old_data(
        ...
        for category_name, retention_period in retention_policies.items():
            try:
                ...
                collection_name = self.collection_manager.collection_configs[category][
                    "name"
                ]
                collection = self.db[collection_name]
...
def create_mongodb_manager(connection_string: str, **kwargs) -> EnhancedMongoDBManager:
    ...
    config = MongoDBConfig(connection_string=connection_string, **kwargs)
...
if __name__ == "__main__":
    ...
    async def test_mongodb_manager():
        ...
        # Query data
        query_result = await manager.query_market_data(symbols=["AAPL"], limit=10)

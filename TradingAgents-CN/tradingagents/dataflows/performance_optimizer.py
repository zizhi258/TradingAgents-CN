...
class PerformanceMonitor:
            ...
            for container in containers:
                if "tradingagents" in container.name:
                    try:
                        ...
                        self.logger.warning(
                            f"Failed to collect stats for {container.name}: {e}"
                        )
    ...
    def _collect_queue_metrics(self) -> dict[str, float]:
        ...
        return {"queue_size": 100, "queue_processing_rate": 50, "queue_latency_ms": 100}
    ...
    def _map_metric_to_resource_type(self, metric_name: str) -> ResourceType | None:
        """Map metric name to resource type"""
...
class ResourceOptimizer:
    ...
    async def apply_optimization(
        ...
        try:
            ...
            if success:
                ...
                self.monitor.performance_stats[
                    "cost_savings"
                ] += recommendation.cost_impact
                self.monitor.performance_stats["last_optimization_time"] = (
                    datetime.now()
                )
    ...
    async def _scale_down_resources(
        self, recommendation: OptimizationRecommendation
    ) -> bool:
        ...
        for resource, usage in current_usage.items():
            if resource.endswith("_cores") or resource.endswith("_gb"):
                ...
                if usage < current_usage.get(f"{resource}_target", usage * 0.7):
                    potential_savings = (
                        usage - current_usage.get(f"{resource}_target", usage * 0.7)
                        ...
                        recommendations.append(
                            {
                                "resource": resource,
                                "current_usage": usage,
                                "recommended_usage": current_usage.get(
                                    f"{resource}_target", usage * 0.7
                                ),
...
def create_resource_optimizer(
    config: dict[str, Any], monitor: PerformanceMonitor
) -> ResourceOptimizer:
...
if __name__ == "__main__":
    ...
    async def test_performance_optimization():
        ...
        print(
            f"Alerts sent: {summary.get('performance_stats', {}).get('alerts_sent', 0)}"
        )

...
class StreamingDataManager:
    ...
    async def start_market_data_stream(
        self, symbols: list[str], sources: list[str] = None
    ):
    ...
    async def _start_finnhub_stream(self, symbols: list[str]):
        ...
        try:
            ...
            def on_close(ws, close_status_code, close_msg):
                ...
                threading.Timer(
                    5.0, lambda: self._start_finnhub_stream(symbols)
                ).start()
    ...
    async def start_news_stream(
        self, sources: list[str] = None, keywords: list[str] = None
    ):
            ...
            for symbol in symbols:
                message = StreamMessage(
                    ...
                    timestamp=datetime.fromtimestamp(
                        article_data.get("datetime", time.time())
                    ),
    ...
    async def _update_technical_indicators(self, message: StreamMessage):
        ...
        try:
            ...
            await redis_client.zadd(
                price_series_key, {json.dumps(price_point): timestamp.timestamp()}
            )
    ...
    async def _calculate_real_time_indicators(
        ...
        try:
            ...
            price_series_key = f"price_series:{symbol}"
            recent_data = await redis_client.zrevrange(
                price_series_key, 0, 49, withscores=True
            )
...
class MessageHandlers:
    ...
    async def handle_market_tick(message: StreamMessage):
        ...
        try:
            ...
            logging.getLogger("message_handlers").error(
                f"Market tick handler error: {e}"
            )
    ...
    async def handle_news_article(message: StreamMessage):
        ...
        try:
            ...
            sentiment_score = await MessageHandlers._analyze_sentiment(
                message.data.get("headline", "")
            )
            message.data["sentiment_score"] = sentiment_score

            # Store in news database
            from tradingagents.dataflows.mongodb_integration import (
...
def create_streaming_pipeline(config: dict[str, Any]) -> StreamingDataManager:
    ...
    pipeline.register_message_handler(
        StreamingDataType.MARKET_TICK, MessageHandlers.handle_market_tick
    )
    pipeline.register_message_handler(
        StreamingDataType.NEWS_ARTICLE, MessageHandlers.handle_news_article
    )
    ...
    async def main():
        ...
        try:
            while True:
                ...
                print(
                    f"Processing rate: {status['processing_stats']['processing_rate']:.2f} msg/s"
                )

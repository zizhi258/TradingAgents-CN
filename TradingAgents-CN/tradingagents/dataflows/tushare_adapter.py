...
class TushareDataAdapter:
    ...
    def _get_daily_data(self, symbol: str, start_date: str = None, end_date: str = None) -> pd.DataFrame:
        ...
        logger.info(f"🔍 [股票代码追踪] _get_daily_data 优先调用 provider.get_stock_daily_probar (qfq)，传入: symbol='{symbol}'")
        import time
        ...
        if data is None or (hasattr(data, 'empty') and data.empty):
            logger.info(f"🔄 [TushareAdapter详细日志] 回退到 provider.get_stock_daily")
        ...
        try:
            ...
            try:
                ...
                if 'amount' in standardized.columns and had_amount:
                    ...
                    logger.info("✅ [单位对齐] amount 已从‘千元’换算为‘人民币元’（×1000）")

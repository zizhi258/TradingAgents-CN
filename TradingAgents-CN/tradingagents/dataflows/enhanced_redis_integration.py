...
class EnhancedRedisManager:
    ...
    async def _initialize_connections(self):
        ...
        try:
            ...
            for cluster_name, redis_url in redis_configs.items():
                ...
                self.logger.info(
                    f"Redis cluster '{cluster_name}' connected successfully"
                )
    ...
    async def set_market_data(
        ...
        try:
            ...
            if len(self.l1_cache) > self.cache_config.max_memory_items:
                oldest_key = min(self.l1_cache_timestamps.items(), key=lambda x: x[1])[
                    0
                ]
    ...
    async def get_market_data(
        self, symbol: str, cluster: str = "primary"
    ) -> dict[str, Any] | None:
    ...
    async def get_cached_analysis(
        self, symbol: str, analysis_type: str, cluster: str = "primary"
    ) -> dict[str, Any] | None:
    ...
    async def get_ml_features(
        self, symbol: str, feature_set: str = "default", cluster: str = "primary"
    ) -> dict[str, Any] | None:
            ...
            async with self._operation_context("write"):
                if delay > 0:
                    ...
                    await redis_client.zadd(
                        delayed_key, {json.dumps(task_payload): execute_timestamp}
                    )
                else:
                    # Use immediate priority queue
                    await redis_client.zadd(
                        queue_key, {json.dumps(task_payload): priority}
                    )
    ...
    async def dequeue_tasks(
        self, queue_name: str, batch_size: int = 1, cluster: str = "primary"
    ) -> list[dict[str, Any]]:
        ...
        try:
            ...
            async with self._operation_context("stream"):
                message_id = await redis_client.xadd(
                    stream_key, data, maxlen=max_len, approximate=True
                )
    ...
    async def rate_limit_check(
        self, identifier: str, limit: int, window_seconds: int, cluster: str = "primary"

...
class AnomalyDetector:
    ...
    def fit_symbol_model(self, symbol: str, historical_data: list[dict[str, Any]]):
        ...
        try:
            if len(historical_data) < self.min_samples:
                logging.warning(
                    f"Insufficient data for {symbol} anomaly model: {len(historical_data)} samples"
                )
                return
            ...
            if len(features) < self.min_samples:
                logging.warning(
                    f"Insufficient clean data for {symbol}: {len(features)} samples"
                )
                return
            ...
            logging.info(
                f"Anomaly model trained for {symbol} with {len(features)} samples"
            )
    ...
    def detect_anomalies(
        ...
        try:
            ...
            anomalies = []
            for i, (idx, score, pred) in enumerate(
                zip(clean_indices, anomaly_scores, predictions, strict=False)
            ):
                if pred == -1:  # Anomaly detected
                    ...
                    for j, col in enumerate(feature_cols):
                        ...
                        # Calculate z-score
                        z_score = (
                            (value - base_stats["mean"]) / base_stats["std"]
                            if base_stats["std"] > 0
                            else 0
                        )
                        ...
                        if value < base_stats["p25"]:
                            percentile_dev = (
                                (base_stats["p25"] - value)
                                / (base_stats["p25"] - base_stats["min"])
                                if base_stats["p25"] != base_stats["min"]
                                else 0
                            )
                        elif value > base_stats["p75"]:
                            percentile_dev = (
                                (value - base_stats["p75"])
                                / (base_stats["max"] - base_stats["p75"])
                                if base_stats["max"] != base_stats["p75"]
                                else 0
                            )
                        else:
                            percentile_dev = 0
    ...
    def update_model(self, symbol: str, new_data: list[dict[str, Any]]):
        ...
        try:
            ...
            if symbol in self.models:
                ...
                logging.info(
                    f"Updated anomaly model for {symbol} with {len(new_data)} new samples"
                )
...
class DataQualityMonitor:
    ...
    async def monitor_data_quality(
        ...
        try:
            self.logger.info(
                f"Starting data quality monitoring for {dataset_name} with {len(data)} records"
            )
            ...
            for metric in self.quality_metrics:
                if metric.enabled:
                    try:
                        result = await self._execute_quality_metric(
                            metric, data, metadata
                        )
                        quality_results.append(result)

                        # Update quality history
                        self.quality_history[f"{dataset_name}_{metric.name}"].append(
                            result.value
                        )
            ...
            self.logger.info(
                f"Data quality monitoring completed for {dataset_name}. Score: {overall_score:.2f}"
            )
    ...
    async def _execute_quality_metric(
        self,
    ...
    def _check_completeness(
        self, metric: QualityMetric, data: list[dict[str, Any]]
    ) -> QualityResult:
        ...
        if metric.name == "data_completeness_required_fields":
            ...
            for field in optional_fields:
                ...
                field_completeness[field] = (
                    complete_count / total_records if total_records > 0 else 0
                )

            avg_completeness = (
                statistics.mean(field_completeness.values())
    ...
    def _check_consistency(
        ...
        if metric.name == "ohlc_consistency":
            ...
            consistency_score = (
                consistent_records / total_records if total_records > 0 else 0
            )
        ...
        if metric.name == "data_freshness":
            ...
            return QualityResult(
                ...
                details={
                    ...
                    "total_records": total_records,
                    "freshness_threshold_hours": freshness_threshold.total_seconds()
                    / 3600,
                },
                timestamp=datetime.now(),
    ...
    def _check_validity(
        ...
        if metric.name == "data_type_validity":
            ...
            for record in data:
                ...
                if (
                    total_fields > 0 and valid_fields / total_fields >= 0.9
                ):  # 90% of fields valid
                    valid_records += 1
    ...
    def _check_uniqueness(
        ...
        if metric.name == "duplicate_records":
            ...
            uniqueness_score = (
                unique_records / total_records if total_records > 0 else 0
            )
    ...
    async def _detect_data_anomalies(
        ...
        try:
            ...
            self.logger.info(
                f"Detected {len(all_anomalies)} anomalies in {dataset_name}"
            )
    ...
    def _generate_summary(
        self, quality_results: list[QualityResult], anomalies: list[dict[str, Any]]
    ) -> dict[str, Any]:
        ...
        for result in quality_results:
            ...
            if "completeness" in result.metric_name:
                ...
                "timeliness" in result.metric_name or "freshness" in result.metric_name
            ):
    ...
    def _generate_recommendations(
        ...
        if any("accuracy" in r.metric_name for r in failed_metrics):
            recommendations.append(
                "Implement stricter data validation at ingestion point"
            )

        if any("consistency" in r.metric_name for r in failed_metrics):
            recommendations.append(
                "Add business rule validation to ensure data consistency"
            )

        if any("timeliness" in r.metric_name for r in failed_metrics):
            recommendations.append(
                "Optimize data processing pipelines to improve freshness"
            )

        if len(anomalies) > 0:
            recommendations.append(
                f"Investigate {len(anomalies)} detected anomalies for potential data quality issues"
            )
        ...
        if overall_score < 0.8:
            recommendations.append(
                "Consider implementing automated data cleansing procedures"
            )
            recommendations.append(
    ...
    async def _check_and_send_alerts(self, report: DataQualityReport):
        ...
        try:
            ...
            critical_issues = [
                ...
                if not m.passed
                and m.severity
                in [DataQualitySeverity.CRITICAL, DataQualitySeverity.ERROR]
            ]
            ...
            if critical_issues:
                ...
                if (
                    last_alert_time and (current_time - last_alert_time).seconds < 1800
                ):  # 30 minutes cooldown
                    return
                ...
                alert_message = {
                    ...
                    "issues": [
                        f"{issue.metric_name}: {issue.message}"
                ...
                self.logger.warning(
                    f"Data quality alert: {json.dumps(alert_message, indent=2)}"
                )
    ...
    def get_quality_trends(
        self, dataset_name: str, metric_name: str, days: int = 30
    ) -> dict[str, Any]:
    ...
    def get_monitoring_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "alert_history_size": sum(
                len(alerts) for alerts in self.alert_history.values()
            ),
            "quality_history_size": sum(
                len(history) for history in self.quality_history.values()
            ),
        }
...
if __name__ == "__main__":
    ...
    async def test_quality_monitor():
        config = {"anomaly_contamination": 0.05, "reports_dir": "/tmp/quality_reports"}

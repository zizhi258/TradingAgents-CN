...
class ChartGenerationRequest(BaseModel):
    ...
    chart_types: list[str] | None = Field(None, description="指定图表类型")
    market_data: dict[str, Any] | None = Field(None, description="市场数据")
    config: dict[str, Any] | None = Field(None, description="图表配置")
...
async def generate_visualization(
    request: ChartGenerationRequest, background_tasks: BackgroundTasks
        ...
        for chart_info in viz_results.get("charts_generated", []):
            chart_response = ChartResponse(
                ...
                title=chart_info.get(
                    "title", f"{request.symbol} {chart_info['chart_type']}"
                ),
...
async def list_charts(
    ...
    chart_type: str | None = Query(None, description="图表类型过滤"),
    ...
    try:
        ...
        return ChartListResponse(
            charts=paginated_charts, total=total, page=page, page_size=page_size
        )
...
async def delete_chart(filename: str) -> dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": f"图表文件 {filename} 已删除"}
    ...
    try:
        ...
        return {
            ...
            "configuration": {
                ...
                "default_theme": charting_artist.chart_config.get("charting_artist", {})
                .get("chart_settings", {})
...
async def batch_generate_charts(
    requests: list[ChartGenerationRequest], background_tasks: BackgroundTasks

...
class SiliconFlowClient(BaseMultiModelAdapter):
    ...
    def __init__(self, config: dict[str, Any]):
        ...
        self.r1_timeout = int(
            config.get("r1_timeout") or os.getenv("SILICONFLOW_R1_TIMEOUT", 180)
        )
    ...
    def initialize_client(self) -> None:
        ...
        logger.info(
            f"SiliconFlow客户端初始化完成，支持{len(self._supported_models)}个模型"
        )
    ...
    def execute_task(
        ...
        try:
            ...
            request_data = {
                ...
                "max_tokens": min(
                    model_spec.max_tokens, kwargs.get("max_tokens", 4096)
                ),
                "temperature": kwargs.get(
                    "temperature", self._get_optimal_temperature(task_spec)
                ),
            ...
            if streaming and callable(on_token):
                ...
                logger.info(
                    f"任务执行成功(流式): {model_name}, 耗时: {execution_time}ms"
                )
                return TaskResult(
                    result=full_text,
                    model_used=model_spec,
                ...
                if "choices" in response_data and len(response_data["choices"]) > 0:
                    ...
                    logger.info(
                        f"任务执行成功: {model_name}, 耗时: {execution_time}ms, 成本: ${actual_cost:.6f}"
                    )

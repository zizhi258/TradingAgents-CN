...
class CollaborativeAnalysisRequest(BaseModel):
    ...
    trade_date: str | None = Field(None, description="交易日期，格式：YYYY-MM-DD")
    collaboration_mode: str = Field(
        "sequential", description="协作模式：sequential/parallel/debate"
    )
    selected_agents: list[str] | None = Field(None, description="选择的智能体列表")
    context: dict[str, Any] | None = Field(None, description="额外的上下文信息")
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "context": {"urgency": "normal", "complexity": "medium"},
            }
        }
...
class SmartAnalysisRequest(BaseModel):
    ...
    ticker: str = Field(..., description="股票代码")
    trade_date: str | None = Field(None, description="交易日期，格式：YYYY-MM-DD")
    context: dict[str, Any] | None = Field(None, description="分析上下文")
...
class ModelSelectionRequest(BaseModel):
    ...
    task_description: str | None = Field("", description="任务描述")
    ...
    def _setup_routes(self):
        ...
        async def collaborative_analysis(request: CollaborativeAnalysisRequest):
            ...
            try:
                ...
                # 执行协作分析
                logger.info(
                    f"开始协作分析: {request.ticker}, 模式: {request.collaboration_mode}"
                )
                ...
                return {"success": True, "data": result, "message": "协作分析完成"}
                ...
                raise HTTPException(
                    status_code=500, detail=f"协作分析执行失败: {str(e)}"
                )
        ...
        async def smart_analysis(request: SmartAnalysisRequest):
            ...
            try:
                ...
                return {"success": True, "data": result, "message": "智能分析完成"}

            except Exception as e:
                logger.error(f"智能分析失败: {e}", exc_info=True)
                raise HTTPException(
                    status_code=500, detail=f"智能分析执行失败: {str(e)}"
                )
        ...
        async def select_optimal_model(request: ModelSelectionRequest):
            ...
            try:
                ...
                multi_model_manager = (
                    self.trading_graph.multi_model_extension.multi_model_manager
                )
        ...
        async def get_system_status():
            ...
            try:
                ...
                return {"success": True, "data": status, "message": "系统状态获取成功"}

            except Exception as e:
                logger.error(f"获取系统状态失败: {e}", exc_info=True)
                raise HTTPException(
                    status_code=500, detail=f"获取系统状态失败: {str(e)}"
                )
        ...
        async def get_available_agents():
            ...
            try:
                ...
                collaboration_modes = (
                    self.trading_graph.get_available_collaboration_modes()
                )
                ...
                if self.trading_graph.multi_model_extension:
                    for agent_name in agents:
                        if (
                            agent_name
                            in self.trading_graph.multi_model_extension.specialized_agents
                        ):
                            agent = self.trading_graph.multi_model_extension.specialized_agents[
                                agent_name
                            ]
                ...
                raise HTTPException(
                    status_code=500, detail=f"获取智能体列表失败: {str(e)}"
                )

        @self.router.get("/performance/metrics")
        async def get_performance_metrics(
            time_window_hours: int = Query(
                24, description="时间窗口（小时）", ge=1, le=720
            )
        ):
            """获取性能指标"""
            try:
                ...
                routing_engine = (
                    self.trading_graph.multi_model_extension.multi_model_manager.routing_engine
                )
                ...
                for (
                    ...
                    self.trading_graph.multi_model_extension.specialized_agents.items()
                ):
        ...
        async def enable_smart_analysis():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"启用智能分析模式失败: {str(e)}"
                )
        ...
        async def get_available_models():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"获取模型列表失败: {str(e)}"
                )

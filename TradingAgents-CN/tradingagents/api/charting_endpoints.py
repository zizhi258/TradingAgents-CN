...
redis_client = redis.from_url(
    os.getenv("TRADINGAGENTS_REDIS_URL", "redis://localhost:6379")
)
...
class ChartGenerationRequest(BaseModel):
    ...
    render_mode: str | None = Field(
        default=None, description="LLM绘图渲染模式: python|html"
    )
    model_override: str | None = Field(
        default=None, description="强制指定LLM模型，如 moonshotai/Kimi-K2-Instruct"
    )
...
def _build_default_mm_config() -> dict[str, Any]:
    ...
    return {
        "deepseek": {
            "enabled": os.getenv("DEEPSEEK_ENABLED", "true").lower()
            in ("1", "true", "yes", "on"),
        ...
        "siliconflow": {
            "enabled": os.getenv("SILICONFLOW_ENABLED", "false").lower()
            in ("1", "true", "yes", "on"),
...
async def generate_charts(
    ...
    try:
        ...
        complexity_score = _calculate_complexity(
            request.chart_types, request.data_sources
        )

        if complexity_score > 3 or request.priority == "low":
            # 异步生成
            background_tasks.add_task(
                _generate_charts_async, task_id, request.dict(), charting_artist
            )
            ...
            result = charting_artist.generate_visualizations(
                ...
                runtime_config={
                    ...
                    "render_mode": (
                        request.render_mode
                        or os.getenv("CHARTING_ARTIST_RENDER_MODE", "python")
                    ),
    ...
    try:
        ...
        if not charts_dir.exists():
            return ChartListResponse(
                charts=[], total=0, page=page, page_size=page_size, has_next=False
            )

        # 查找匹配的图表文件
        chart_files = list(charts_dir.glob(f"*{analysis_id}*.html")) + list(
            charts_dir.glob(f"*{analysis_id}*.json")
        )
...
async def delete_chart(chart_id: str):
    ...
    try:
        ...
        return {"message": f"成功删除图表 {chart_id}", "deleted_files": deleted_files}
...
async def batch_operations(
    request: BatchOperationRequest, background_tasks: BackgroundTasks
):
    """批量图表操作"""
    try:
        # 生成批量任务ID
        ...
        # 将批量操作加入后台任务
        background_tasks.add_task(
            _process_batch_operations, batch_id, request.operations
        )
...
async def list_all_charts(
    ...
    chart_type: str | None = Query(None, description="按图表类型筛选"),
    created_after: datetime | None = Query(None, description="创建时间过滤"),
):
    """列出所有图表"""
    try:
        charts_dir = Path(os.getenv("CHART_STORAGE_PATH", "data/attachments/charts"))
...
async def health_check():
    ...
    try:
        ...
        charts_dir = Path(os.getenv("CHART_STORAGE_PATH", "data/attachments/charts"))
        storage_status = (
            "healthy" if charts_dir.exists() and charts_dir.is_dir() else "error"
        )
        ...
        if charts_dir.exists():
            ...
            total_size = sum(
                f.stat().st_size for f in charts_dir.iterdir() if f.is_file()
            )
        else:
            chart_count = 0
            total_size = 0
...
async def _generate_charts_async(
    task_id: str, request_data: dict[str, Any], charting_artist: ChartingArtist
):
    """异步生成图表"""
    try:
        # 更新任务状态
        ...
        result = charting_artist.generate_visualizations(
            ...
            runtime_config={
                "llm_enabled": True,
                "render_mode": request_data.get("render_mode")
                or os.getenv("CHARTING_ARTIST_RENDER_MODE", "python"),
...
async def _process_batch_operations(batch_id: str, operations: list[dict[str, Any]]):
    ...
    try:
        ...
        for i, operation in enumerate(operations):
            try:
                ...
                await _update_batch_status(
                    batch_id,
                    "processing",
                    f"处理中 ({completed}/{len(operations)})",
                    results,
                    progress,
                )
        ...
        await _update_batch_status(
            batch_id,
            "completed",
            f"批量操作完成 ({completed}/{len(operations)})",
            results,
            100,
        )
...
async def _update_task_status(
    task_id: str, status: str, message: str, result: dict[str, Any] = None
):
    """更新任务状态"""
    task_data = {
        "task_id": task_id,

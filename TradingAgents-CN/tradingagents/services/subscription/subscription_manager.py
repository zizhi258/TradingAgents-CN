...
class SubscriptionManager:
        ...
        if self.storage:
            try:
                ...
                self.storage.db[self.collection].create_index(
                    [("email", 1), ("symbol", 1), ("status", 1)]
                )
                self.storage.db[self.collection].create_index(
                    [("market_type", 1), ("frequency", 1)]
                )
                # 新增：类型与范围相关索引
                self.storage.db[self.collection].create_index(
                    [
    ...
    def remove_subscription(self, email: str, symbol: str = None) -> int:
        ...
        if self.storage:
            ...
            for sub in self._memory_storage:
                if sub.get("email") == email.lower() and sub.get("status") == "active":
    ...
    def get_subscriptions_by_market(self, exchanges: list[str]) -> list[dict]:
        ...
        market_types = list(
            set(market_map.get(ex) for ex in exchanges if ex in market_map)
        )
        ...
        if self.storage:
            ...
            subscriptions = [
                ...
                for sub in self._memory_storage
                if (
                    sub.get("market_type") in market_types
                    and sub.get("frequency") == "close"
                    and sub.get("status") == "active"
                    and (sub.get("subscription_type") in (None, "stock"))
                )
            ]
    ...
    def update_subscription(self, email: str, symbol: str, updates: dict) -> bool:
        """更新订阅设置
        ...
        query = {"email": email.lower(), "symbol": symbol.upper(), "status": "active"}

        if self.storage:
            result = self.storage.update_one(self.collection, query, {"$set": updates})
            return result.modified_count > 0
        else:
            # 内存存储模式
            for sub in self._memory_storage:
                if (
                    sub.get("email") == email.lower()
                    and sub.get("symbol") == symbol.upper()
                    and sub.get("status") == "active"
                ):
                    sub.update(updates)
                    return True
    ...
    def mark_as_sent(self, subscription_id: str):
        ...
        Args:
            subscription_id: 订阅ID
        """
        updates = {"last_sent": datetime.now(), "$inc": {"send_count": 1}}

        if self.storage:
            self.storage.update_one(self.collection, {"_id": subscription_id}, updates)
        else:
            # 内存存储模式
            for sub in self._memory_storage:
    ...
    def get_subscription_stats(self) -> dict:
        ...
        if self.storage:
            ...
            # 内存存储模式
            active_subs = [
                s for s in self._memory_storage if s.get("status") == "active"
            ]
            ...
            frequency_stats = [
                {"_id": k, "count": v} for k, v in frequency_stats.items()
            ]
        ...
        if self.storage:
            try:
                pipeline = [
                    ...
                    {
                        "$group": {
                            "_id": {"$ifNull": ["$subscription_type", "stock"]},
                            "count": {"$sum": 1},
                        }
                    },
                ]
    ...
    def _find_subscription(self, email: str, symbol: str) -> dict | None:
        """查找订阅"""
        query = {"email": email.lower(), "symbol": symbol.upper()}
    ...
    def _find_market_subscription(self, email: str, market_scope: str) -> dict | None:
        """查找市场级订阅（按邮箱+范围唯一）"""
        query = {
            "email": email.lower(),
        ...
        if self.storage:
            ...
            for sub in self._memory_storage:
                if (
                    sub.get("email") == email.lower()
                    and sub.get("subscription_type") == "market"
                    and sub.get("market_scope") == market_scope
                ):
                    return sub
            return None

    def _find_index_subscription(self, email: str, index_code: str) -> dict | None:
        """查找指数订阅（按邮箱+指数代码唯一）。"""
        query = {
            "email": email.lower(),
        ...
        if self.storage:
            ...
            for sub in self._memory_storage:
                if (
                    sub.get("email") == email.lower()
                    and sub.get("subscription_type") == "index"
                    and sub.get("symbol") == index_code.upper()
                ):
                    return sub
            return None
        ...
        if self.storage:
            ...
            subscriptions = [
                ...
                if (
                    ...
                    and (
                        not frequency_filter or sub.get("frequency") in frequency_filter
                    )
                    and (
                        not subscription_type
                        or sub.get("subscription_type") == subscription_type
                    )
                )
            ]
    ...
    def get_market_subscriptions(
        self, *, scope: str | None = None, frequency_filter: list[str] | None = None
    ) -> list[dict]:
        """获取市场级订阅列表
        ...
        subs = self.get_active_subscriptions(
            frequency_filter=frequency_filter, subscription_type="market"
        )
        if scope:
            subs = [s for s in subs if s.get("market_scope") == scope]
        return subs

    def get_index_subscriptions(
        self, *, market: str | None = None, frequency_filter: list[str] | None = None
    ) -> list[dict]:
        """获取指数订阅列表。
        ...
        """
        subs = self.get_active_subscriptions(
            frequency_filter=frequency_filter, subscription_type="index"
        )
        if market:
    ...
    def list_common_index_options(self) -> list[dict]:
        ...
        try:
            # 首选从API模块导入内置映射
            from tradingagents.api.market_data_endpoints import (
                INDEX_PRESETS,  # type: ignore
            )

...
class ModelCache:
    ...
    def __init__(self, max_models: int = 10, max_memory_mb: float = 2048):
        ...
        logger.info(
            f"Model cache initialized: max_models={max_models}, max_memory={max_memory_mb}MB"
        )
    ...
    def cache_model(self, model_key: str, model: Any, size_mb: float = None):
        ...
        with self._lock:
            ...
            if size_mb is None:
                try:
                    if hasattr(model, "state_dict"):  # PyTorch model
                        size_mb = sum(p.numel() * 4 for p in model.parameters()) / (
                            1024 * 1024
                        )  # Assume float32
                    else:
            ...
            while (
                ...
                or current_memory + size_mb > self.max_memory_mb
            ):
        ...
        with self._lock:
            return {
                ...
                "average_model_size_mb": (
                    np.mean(list(self.model_sizes.values())) if self.model_sizes else 0
                ),
            }
...
class BatchInferenceEngine:
    ...
    def _batch_processing_loop(self):
        ...
        while self.is_running:
            try:
                ...
                should_process = len(batch_requests) >= self.max_batch_size or (
                    batch_requests
    ...
    def _process_model_batch(self, model_key: str, requests: list[dict]):
        ...
        try:
            ...
            logger.debug(
                f"Processed batch of {len(requests)} for {model_key} in {inference_time*1000:.1f}ms"
            )
...
class ModelServingOptimizer:
    ...
    def _load_model(self, model_key: str) -> Any | None:
    ...
    async def predict_async(
        ...
        try:
            ...
            # Perform prediction
            if use_batch:
                result = await self.batch_engine.infer_async(
                    model_key, model, input_data
                )
                with self._metrics_lock:
                ...
                result = await loop.run_in_executor(
                    self.async_executor, model.predict, input_data
                )
    ...
    def predict_batch(
        self, model_key: str, input_batch: list[Any], use_cache: bool = True
    ) -> list[Any]:
        ...
        try:
            ...
            if use_cache:
                for i, input_data in enumerate(input_batch):
                    f"prediction:{model_key}:{hash(str(input_data))}"
            ...
            if uncached_inputs:
                ...
                if use_cache:
                    for i, input_data in enumerate(uncached_inputs):
                        ...
                        result = (
                            batch_results[i]
                            if hasattr(batch_results, "__getitem__")
                            else batch_results
                        )
    ...
    def preload_models(self, model_keys: list[str] = None):
        ...
        with ThreadPoolExecutor(max_workers=4) as executor:
            futures = {
                executor.submit(self._load_model, key): key for key in model_keys
            }
        ...
        with self._metrics_lock:
            metrics_dict = {
                ...
                "error_rate": (
                    ...
                    / max(self.metrics.inference_count + self.metrics.error_count, 1)
                    * 100
                ),
            }

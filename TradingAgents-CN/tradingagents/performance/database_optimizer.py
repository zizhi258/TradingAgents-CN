...
class ConnectionPoolManager:
    ...
    def get_mongo_client(self, database: str = None) -> MongoClient | None:
    ...
    def get_redis_client(self, db: int = None) -> redis.Redis | None:
    ...
    def mongo_session(self, database: str = None):
        ...
        try:
            ...
            self.pool_stats["mongo"]["peak"] = max(
                self.pool_stats["mongo"]["peak"], self.pool_stats["mongo"]["active"]
            )
            yield session
        finally:
            if session:
    ...
    def redis_pipeline(self, db: int = None):
        ...
        try:
            ...
            self.pool_stats["redis"]["peak"] = max(
                self.pool_stats["redis"]["peak"], self.pool_stats["redis"]["active"]
            )
            yield pipeline
        finally:
            if pipeline:
...
class DatabaseOptimizer:
            ...
            try:
                ...
                for index_spec in indexes:
                    try:
                        ...
                        if (
                            isinstance(index_spec[0], tuple)
                            and index_spec[0][1] == "text"
                        ):
                            # Text index
                            ...
                            logger.info(
                                f"Created text index on {collection_name}.{index_spec[0][0]}"
                            )
                            continue

                        # Regular compound index
                        index_name = (
                            f"opt_{'_'.join([f'{k}_{v}' for k, v in index_spec])}"
                        )

                        # Add partial filter for recent data on stock_data
                        partial_filter = None
                        if collection_name == "stock_data" and "updated_at" in [
                            k for k, v in index_spec
                        ]:
    ...
    async def batch_insert_mongodb(
        ...
        logger.info(
            f"Batch inserted {inserted_count}/{len(documents)} documents to {collection_name}"
        )
        return inserted_count

    def batch_set_redis(
        self, data: dict[str, Any], ttl: int = None, db: int = None
    ) -> int:
    ...
    def execute_optimized_query(
        self, query_func: callable, cache_key: str = None, ttl: int = None
    ) -> Any:
        ...
        try:
            ...
            if execution_time > self.slow_query_threshold:
                logger.warning(
                    f"Slow query detected: {cache_key} took {execution_time:.2f}s"
                )
    ...
    def get_query_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "recent_avg_time": sum(s["execution_time"] for s in recent_queries)
    ...
    def cleanup_query_cache(self, max_age: int = 3600):
        ...
        logger.info(
            f"Cleaned up {len(expired_keys)} expired/excess query cache entries"
        )
        ...
        try:
            ...
            for collection_name in collections:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not get index stats for {collection_name}: {e}"
                    )
    ...
    def create_query_plan_cache(self, database: str = "tradingagents"):
        ...
        for query_spec in common_queries:
            try:
                ...
                logger.warning(
                    f"Query plan warming error for {query_spec['collection']}: {e}"
                )
        ...
        while self.is_running:
            try:
                ...
                if slow_query_count > 10:
                    logger.warning(
                        f"High number of slow queries detected: {slow_query_count}"
                    )

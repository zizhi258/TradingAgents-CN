    ...
    def _setup_prometheus_metrics(self):
        ...
        self.prometheus_metrics = {
            ...
            "api_request_duration": Histogram(
                "trading_api_request_duration_seconds", "API request duration"
            ),
            "cache_hits_total": Counter(
                "trading_cache_hits_total", "Cache hits", ["level"]
            ),
            "cache_misses_total": Counter(
                "trading_cache_misses_total", "Cache misses", ["level"]
            ),
            ...
            "model_inference_duration": Histogram(
                "trading_model_inference_duration_seconds",
                "Model inference duration",
                ["model"],
            ),
            "memory_usage_bytes": Gauge(
                "trading_memory_usage_bytes", "Memory usage in bytes"
            ),
            "cpu_usage_percent": Gauge(
                "trading_cpu_usage_percent", "CPU usage percentage"
            ),
            "active_connections": Gauge(
                "trading_active_connections", "Active connections", ["type"]
            ),
        }
    ...
    def _collect_all_metrics(self):
        ...
        try:
            ...
            if "average_execution_time" in db_stats:
                self.record_metric(
                    "db.avg_query_time", db_stats["average_execution_time"] * 1000
                )  # ms
            ...
            if "serving_metrics" in model_stats:
                ...
                if "avg_inference_time_ms" in serving:
                    self.record_metric(
                        "model.avg_inference_time", serving["avg_inference_time_ms"]
                    )
                if "cache_hit_rate" in serving:
                    self.record_metric(
                        "model.cache_hit_rate", serving["cache_hit_rate"]
                    )
            ...
            if "resource_metrics" in resource_stats:
                if "current_metrics" in resource_stats["resource_metrics"]:
                    ...
                    if current:
                        ...
                        self.record_metric(
                            "system.memory_percent", current.memory_percent
                        )
    ...
    def _update_prometheus_metrics(self):
        ...
        try:
            ...
            for metric_name, buffer in self.metrics.items():
                ...
                if (
                    "cpu_percent" in metric_name
                    and "cpu_usage_percent" in self.prometheus_metrics
                ):
                    self.prometheus_metrics["cpu_usage_percent"].set(latest.value)
                elif (
                    "memory_percent" in metric_name
                    and "memory_usage_bytes" in self.prometheus_metrics
                ):
                    # Convert percentage to bytes (approximate)
                    self.prometheus_metrics["memory_usage_bytes"].set(
                        latest.value * 1024 * 1024 * 100
                    )  # Rough estimate
...
class AlertManager:
    ...
    def _check_rule(self, rule_id: str, rule: dict[str, Any]):
        ...
        if triggered:
            ...
            if rule_id in self.alert_cooldowns:
                ...
                if (datetime.now() - last_alert_time).total_seconds() < rule[
                    "cooldown"
                ]:
                    return
            ...
            alert = Alert(
                ...
                message=rule["message"].format(
                    value=current_value, threshold=threshold, metric=metric_name
                ),
                metric_name=metric_name,
    ...
    def _trigger_alert(self, alert: Alert):
        ...
        logger.log(
            (
                50 if alert.severity == AlertSeverity.CRITICAL else 40
            ...
            if metric_name in self.metrics_collector.metrics:
                ...
                if latest:
                    ...
                    if metric_name == "api.avg_response_time" and latest.value > 2000:
                        overall_health = (
                            "warning" if overall_health == "good" else overall_health
                        )

...
class UpstreamContributionPreparer:
    ...
    def __init__(
        self, source_dir: str = ".", target_dir: str = "./upstream_contribution"
    ):
    ...
    def analyze_chinese_content(self) -> dict[str, list[str]]:
        ...
        for file_path in self.source_dir.rglob("*.py"):
            ...
            try:
                ...
                if chinese_lines:
                    chinese_files[str(file_path.relative_to(self.source_dir))] = (
                        chinese_lines
                    )
    ...
    def clean_chinese_content(self, file_path: Path, target_path: Path):
        ...
        try:
            ...
            # 替换中文注释
            content = re.sub(
                r"#\s*[\u4e00-\u9fff].*", "# TODO: Add English comment", content
            )
...
"""
    ...
    def generate_contribution_summary(self):
        ...
        with open(
            self.target_dir / "contribution_summary.json", "w", encoding="utf-8"
        ) as f:
            json.dump(summary, f, indent=2)

        logger.info(
            f"📊 Generated summary: {self.target_dir / 'contribution_summary.json'}"
        )
    ...
    def prepare_all_batches(self):
        ...
        if chinese_files:
            ...
            with open(
                self.target_dir / "chinese_content_analysis.json", "w", encoding="utf-8"
            ) as f:

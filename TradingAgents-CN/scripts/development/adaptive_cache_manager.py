...
# 导入智能配置
try:
    from smart_config import get_smart_config
...
class AdaptiveCacheManager:
        ...
        if self.mongodb_enabled:
            try:
                ...
                self.mongodb_client = pymongo.MongoClient(
                    "localhost", 27017, serverSelectionTimeoutMS=2000
                )
                # 测试连接
        ...
        if self.redis_enabled:
            try:
                ...
                self.redis_client = redis.Redis(
                    host="localhost", port=6379, socket_timeout=2
                )
                # 测试连接
    ...
    def save_stock_data(
        ...
        cache_key = self._get_cache_key(
            symbol, start_date or "", end_date or "", data_source
        )

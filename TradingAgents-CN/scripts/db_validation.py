    ...
    def _connect_databases(self):
        ...
        try:
            ...
            self.mongo_client = MongoClient(
                self.mongodb_url, serverSelectionTimeoutMS=10000
            )
        ...
        try:
            ...
            if missing_collections:
                ...
                result["issues"].append(
                    f"Missing collections: {', '.join(missing_collections)}"
                )
            ...
            if collection_name == "system_info":
                ...
                for config_key in required_configs:
                    if not collection.find_one({"key": config_key}):
                        issues.append(
                            f"{collection_name}: Missing config '{config_key}'"
                        )
    ...
    def _validate_indexes(self) -> dict[str, Any]:
        ...
        try:
            ...
            for collection_name, expected in expected_indexes.items():
                ...
                existing = [
                    idx["name"] for idx in self.db[collection_name].list_indexes()
                ]
        ...
        try:
            ...
            if invalid_dates:
                integrity_checks.append(
                    f"Invalid dates found in: {', '.join(invalid_dates)}"
                )
    ...
    def _check_duplicates(self) -> dict[str, int]:
        ...
        try:
            ...
            if "configurations" in self.db.list_collection_names():
                config_duplicates = list(
                    self.db.configurations.aggregate(
                        [
    ...
    def _validate_performance(self) -> dict[str, Any]:
        ...
        try:
            ...
            if slow_queries:
                result["status"] = "warning"
                result["issues"].append(
                    f"Slow queries detected in: {', '.join(slow_queries)}"
                )
            else:
                result["status"] = "pass"

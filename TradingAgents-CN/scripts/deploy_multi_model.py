...
def check_prerequisites() -> dict[str, bool]:
    ...
    print(
        f"{status} Python版本: {python_version.major}.{python_version.minor} ({'符合要求' if checks['python_version'] else '需要3.8+'})"
    )
...
def setup_environment_config() -> bool:
    ...
    if "MULTI_MODEL_ENABLED=false" in content:
        content = content.replace(
            "MULTI_MODEL_ENABLED=false", "MULTI_MODEL_ENABLED=true"
        )

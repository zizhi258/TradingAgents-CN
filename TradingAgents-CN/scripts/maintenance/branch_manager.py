...
class BranchManager:
    ...
    def list_branches(self, branch_type=None):
        ...
        remote_branches = self.run_git_command(
        ...
        for btype, config in self.branch_types.items():
            ...
            matching_branches = [
                b for b in local_branches.split("\n") if b.startswith(prefix)
            ]
            ...
            if matching_branches:
                ...
                for branch in matching_branches:
                    ...
                    remote_marker = (
                        " 📤" if f"origin/{branch}" in remote_branches else " 📍本地"
                    )
    ...
    def delete_branch(self, branch_name, force=False):
        ...
        # 检查分支是否已合并
        merged = self.run_git_command(
            f"git branch --merged develop | grep {branch_name}"
        )
...
def main():
    ...
    create_parser.add_argument(
        "type", choices=["feature", "enhancement", "hotfix", "release"], help="分支类型"
    )
    ...
    list_parser.add_argument(
        "-t",
        "--type",
        choices=["feature", "enhancement", "hotfix", "release"],
        help="过滤分支类型",
    )

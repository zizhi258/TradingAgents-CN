                        ...
                        for field in required_fields:
                            if field in schedule:
                                results["passed"].append(
                                    f"✅ {schedule_type}调度字段 {field} 存在"
                                )
                            else:
                                results["failed"].append(
                                    f"❌ {schedule_type}调度字段 {field} 缺失"
                                )
...
def test_manual_trigger_functionality() -> dict[str, Any]:
    ...
    try:
        ...
        trigger_file = create_manual_trigger(
            "daily", {"test": True, "source": "verification_script"}
        )
...
def test_run_reporter() -> dict[str, Any]:
    ...
    try:
        ...
        if run_id:
            ...
            # 测试更新报告
            success = manager.update_run_report(
                run_id, status="running", test_field="test_value"
            )
            if success:

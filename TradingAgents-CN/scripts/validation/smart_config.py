...
class SmartConfigManager:
    ...
    def _detect_mongodb(self) -> tuple[bool, str]:
        ...
        try:
            ...
            # 尝试连接MongoDB
            client = MongoClient(
                "localhost", 27017, serverSelectionTimeoutMS=2000, connectTimeoutMS=2000
            )
    ...
    def _detect_redis(self) -> tuple[bool, str]:
        ...
        try:
            ...
            # 尝试连接Redis
            r = redis.Redis(
                host="localhost", port=6379, socket_timeout=2, socket_connect_timeout=2
            )
            r.ping()
    ...
    def print_status(self):
        ...
        logger.info(
            f"  降级支持: {'启用' if cache_info['fallback_enabled'] else '禁用'}"
        )
...
"""
    ...
    logger.info("2. 或者: .\\set_env.ps1 (设置环境变量)")

...
class ConfigurationManager:
    ...
    def get_config(
        self, env_name: str, include_secrets: bool = True
    ) -> dict[str, Any] | None:
        """
            ...
            try:
                ...
                if rule_type == "required":
                    ...
                    if value and value not in allowed_values:
                        errors.append(
                            f"Invalid value for {field}: must be one of {allowed_values}"
                        )
    ...
    def export_config(self, env_name: str, format_type: str = "env") -> str | None:
        """
    ...
    def get_environment_info(self, env_name: str) -> dict[str, Any] | None:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="TradingAgents-CN Configuration Manager"
    )
    parser.add_argument(
    ...
    create_parser.add_argument(
        "--description", default="", help="Environment description"
    )
    ...
    export_parser = subparsers.add_parser(
        "export", help="Export environment configuration"
    )
    export_parser.add_argument("environment", help="Environment name")
    export_parser.add_argument(
        "--format",
        choices=["env", "json", "yaml", "k8s_secret"],
        default="env",
        help="Export format",
    )
    ...
    if args.command == "list":
        ...
        success = config_manager.create_environment(
            args.name, args.description, base_config
        )
        ...
        if config_output:
            ...
            print(
                f"Failed to export configuration for environment '{args.environment}'"
            )
            exit(1)

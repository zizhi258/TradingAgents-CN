...
class AIIntegrationDemo:
    ...
    async def _demo_ai_initialization(self):
        ...
        self.ai_manager = await initialize_ai_system(
            config_path=None, enable_all_features=True  # Use default
        )
    ...
    async def _demo_llm_orchestrator(self):
        ...
        for i, task in enumerate(test_tasks):
            ...
            if result.success:
                ...
                print(
                    f"      ✅ Success: {model_used} (${cost:.4f}, {execution_time:.2f}s)"
                )
                ...
                llm_results.append(
                    {"task": task, "success": False, "error": result.error_message}
                )
        ...
        if cached_result.success:
            ...
            print(
                f"      {'🎯 Cache HIT' if cached else '🔄 Cache MISS'} ({cache_time:.2f}s)"
            )
    ...
    async def _demo_rag_system(self):
        ...
        for symbol in self.symbols[:2]:  # Limit for demo
            ...
            print(
                f"      📊 {symbol}: {stats['news']} news, {stats['market_data']} market docs"
            )
        ...
        for i, query_info in enumerate(test_queries):
            ...
            print(
                f"      📄 Found {len(rag_response.retrieved_documents)} relevant documents"
            )
        ...
        print(
            f"   📚 Knowledge base: {system_stats['knowledge_base']['total_documents']} documents"
        )
        print(
            f"   🎯 Vector DB: {'Available' if system_stats['rag_system']['vector_db_available'] else 'Fallback mode'}"
        )
        ...
        for report_type, symbols in report_types:
            ...
            try:
                ...
                if "error" not in report_result:
                    ...
                    print(
                        f"      🤖 AI enhanced: {report_result['ai_insights_generated']}"
                    )
    ...
    async def _demo_enhanced_coordination(self):
        ...
        coordination_task = create_market_analysis_task(
            symbol=self.symbols[0], analysis_type="comprehensive"
        )
        ...
        for mode, max_agents in coordination_modes:
            ...
            try:
                ...
                if result.get("success"):
                    print(
                        f"      ✅ Success: {len(result['selected_agents'])} agents participated"
                    )

                    consensus = result.get("consensus_result", {})
                    print(
                        f"      🎯 Consensus score: {consensus.get('consensus_score', 0):.2f}"
                    )
        ...
        self.demo_results["coordination"] = {
            ...
            "successful_coordinations": len(
                [r for r in coordination_results if r.get("success")]
            ),
    ...
    async def _demo_comprehensive_analysis(self):
        ...
        analysis_result = await create_comprehensive_analysis(
            symbol=self.symbols[0], analysis_type="full", ai_manager=self.ai_manager
        )
        ...
        self.demo_results["comprehensive_analysis"] = {
            ...
            "result_summary": {
                "chart_available": "chart_analysis"
                in analysis_result.get("components", {}),
                "news_available": "news_analysis"
                in analysis_result.get("components", {}),
                "risk_available": "risk_assessment"
                in analysis_result.get("components", {}),
    ...
    async def _demo_system_status(self):
        ...
        if self.ai_manager.ai_orchestrator:
            ...
            print(
                f"      • Avg response time: {perf.get('average_response_time', 0):.2f}s"
            )
    ...
    def _save_demo_results(self) -> str:
        ...
        final_results = {"summary": summary, "detailed_results": self.demo_results}
